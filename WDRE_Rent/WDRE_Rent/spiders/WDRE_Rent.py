...
class WDRE_Rent(scrapy.Spider):
    name = 'WDRE_Rent'
    url = "https://www.wdre.com.au/rent/residential-for-rent/"
    prefix = 'https://www.wdre.com.au'
    headers = {
    ...
    count = 1
    db = 'WDRE'
    collection = 'WDRE_Rent'
    # bucket_prefix = f'D_{collection}'
    bucket_prefix = f'Images10'
    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(WDRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2797)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'WES DAVIDSON Real Estate'
            ...
            if response.xpath("//*[contains(text(),'Building Size')]/following-sibling::*[1]/text()").get('').strip():
                land_area = response.xpath("//*[contains(text(),'Building Size')]/following-sibling::*[1]/text()").get('').strip()
