...
class HomBurg_Rent(scrapy.Spider):
    name = "HomBurg_Rent"

    url = "https://homburg.com.au/ajax-search-listings?page=1&property_type%5B%5D=Business&property_type%5B%5D=Residential&property_type%5B%5D=Residential+Land&property_type%5B%5D=Rural&type=Rent"
    prefix = "https://homburg.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(HomBurg_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.xpath("//*[contains(@class,'listing-card')]/a/@href").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://homburg.com.au{each_url}'
        ...
        next_page = response.css(".load-search-more ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2424)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'HomBurg'

            property_street = response.css('.col-sm-12 h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'icon_bed_grey')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'icon_bath_grey')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'icon_car_grey')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('.ld-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.mt20 ::text').getall())  # description
            ...
            prop_images = response.css(".pgwslideshow a ::attr(href)").getall()
            images = []
            ...
            if response.css(".widget-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.widget-agent-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".widget-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css('.widget-agent-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".widget-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath("//*[contains(@src,'icon_land_grey')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
