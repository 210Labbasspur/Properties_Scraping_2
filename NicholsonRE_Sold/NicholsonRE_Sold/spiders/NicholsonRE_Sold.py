...
class NicholsonRE_Sold(scrapy.Spider):
    name = "NicholsonRE_Sold"

    url = "https://www.nicholsonre.com.au/sold-properties/"
    prefix = "https://www.nicholsonre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(NicholsonRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property_img a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2662)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Nicholson Real Estate'

            property_street = response.css('.property-title h2 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Beds:')]/text()").get('').strip().replace('Beds:','').replace(' ','')
            item['Field7'] = response.xpath("//*[contains(text(),'Baths:')]/text()").get('').strip().replace('Baths:','').replace(' ','')
            item['Field8'] = response.xpath("//*[contains(text(),'Garage:')]/text()").get('').strip().replace('Garage:','').replace(' ','')

            price = response.css('.property-price .price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.tab-content .entry-title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@rel,'prettyPhoto')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("#agents .user"):
                agent1 = response.css("#agents .user")[0]
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.css('span ::text').get('').strip()
            ...
            if len(response.css("#agents .user").getall()) > 1:
                agent2 = response.css("#agents .user")[1]
                agent2_name = agent2.css('h2 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('span ::text').get('').strip()
            ...
            if len(response.css("#agents .user").getall()) > 2:
                agent3 = response.css("#agents .user")[2]
                agent3_name = agent3.css('h2 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('span ::text').get('').strip()
            ...
            #
            feature_count = 58
            for feature in response.css(".epl-tax-tax_feature a"):   ##  first 4 features are ID, bed, bath etc.
