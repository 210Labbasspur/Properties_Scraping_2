...
class Booth_Sale(scrapy.Spider):
    name = "Booth_Sale"

    url = "https://www.boothrealestate.com.au/properties-for-sale"
    prefix = "https://www.boothrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Booth_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2405)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Booth Real Estate'

            property_street = response.xpath("//*[@itemprop='streetAddress']/text()").get('').strip()  # property_street
            property_suburb = response.xpath("//*[@itemprop='addressLocality']/text()").get('').strip()  # property_street
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.propertyPage-property-description h2 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".propertyPage-property-description p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='block']/@src").getall()
            images = []
            ...
            if response.css(".agentAvatar"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(src)").get('').strip().replace('100x100','800x800')
                    images_string = f'https://www.boothrealestate.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentAvatar").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(src)").get('').strip().replace('100x100','800x800')
                    images_string = f'https://www.boothrealestate.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentAvatar").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(src)").get('').strip().replace('100x100','800x800')
                    images_string = f'https://www.boothrealestate.com.au{agent_image}' if agent_image else ""
            ...
            bbc = response.css(".propertyPage-property-icons span ::text").getall()
