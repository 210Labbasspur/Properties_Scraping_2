#####################           AreaSpecialistWyndhamCity_Sold
...
class AreaSpecialistWyndhamCity_Sold(scrapy.Spider):
    name = "AreaSpecialistWyndhamCity_Sold"

    url = "https://wyndhamcity.areaspecialist.com.au/data/results/?listing_sale_method=Sale&listing_status=sold&status=sold&gallery=1&pg={}"
    prefix = "https://wyndhamcity.areaspecialist.com.au"
    headers = {
            'accept': '*/*',
            'accept-language': 'en-US,en;q=0.9',
            'priority': 'u=0, i',
            'referer': 'https://wyndhamcity.areaspecialist.com.au/sold',
            ...
            # 'cookie': '_gid=GA1.3.1471741777.1744357208; _ga_BEC3FKP4S2=GS1.1.1744357210.1.1.1744357226.0.0.0; _ga=GA1.3.1648418365.1742575488; _ga_H187HV21PG=GS1.3.1744357212.1.1.1744357229.0.0.0; _WHEELS_AUTHENTICITY=ULaO7W4MM5kryr%2BzTPlA181b3xSu7nRK%2BN9W2wiqoIhswf8srK438ZnIzTcD7pttAvjTh3Qx6S3l32aI%2BDqytf1hrA6ioCpvfQbLo1Md47wgQKpukPYa%2BX%2FuXTsofXRJluh4XmPzOTxTPsAv6cYLcA%3D%3D; FLASH=%7B%7D; drift_campaign_refresh=50bd2e33-85bc-4604-8379-6d8332b4c4ea; _ga_8DR3XGTHDQ=GS1.3.1744436366.2.0.1744436366.0.0.0; drift_aid=cc1f3c87-6e45-47ef-ab1f-cbee1ca4d82c; driftt_aid=cc1f3c87-6e45-47ef-ab1f-cbee1ca4d82c',
        }

    count = 1
    db = 'AreaSpecialistWyndhamCity'
    collection = 'AreaSpecialistWyndhamCity_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(AreaSpecialistWyndhamCity_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        all_links = selector.xpath("//*[@class='listing-slider']/@href").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://wyndhamcity.areaspecialist.com.au{each_url}'
        ...
        if selector.css("div.listing-slider ::attr(href)"):
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2650)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Area Specialist Wyndham-City'

            property_street = response.css('.left h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.left h2 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'bbc-bed-large')]/parent::*[1]/text()").getall() if e.strip())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'bbc-bath-large')]/parent::*[1]/text()").getall() if e.strip())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'bbc-car-large')]/parent::*[1]/text()").getall() if e.strip())

            price = response.css('.linkset:nth-child(1) ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type -')]/text()").get('').strip().replace('Property Type -','')
            ...
            if land_area:
                if 'Acres' in land_area:
                    ...
                    item['Field36'] = land_area.replace('m','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
