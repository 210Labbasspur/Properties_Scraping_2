...
class FrankGordon_Sold(scrapy.Spider):
    name = "FrankGordon_Sold"

    url = "https://www.frankgordon.com.au/recent-sales/"
    prefix = "https://www.frankgordon.com.au"
    headers = {
    ...
    count = 1
    db = 'FrankGordon'
    collection = 'FrankGordon_Sold'
    # bucket_prefix = f'P_{collection}'
    bucket_prefix = f'Images0'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(FrankGordon_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2802)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Frank Gordon Estate Agents'
            ...
            # item['Field6'] = response.xpath("//*[@xxxxxxxxxx='xxxxxxxxxxx']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field6'] = response.xpath("//*[contains(@title,'Beds')]//span[@class='listing-details-value']/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'Baths')]//span[@class='listing-details-value']/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'Car Park')]//span[@class='listing-details-value']/text()").get('').strip()
            ...
            '''          AGENTS          '''
            if response.css(".widget_listing_agent .wpsight-listing-agent"):
                agent1 = response.css(".widget_listing_agent .wpsight-listing-agent")[0]
            ...
            if len(response.css(".widget_listing_agent .wpsight-listing-agent").getall()) > 1:
                agent2 = response.css(".widget_listing_agent .wpsight-listing-agent")[1]
            ...
            if len(response.css(".widget_listing_agent .wpsight-listing-agent").getall()) > 2:
                agent3 = response.css(".widget_listing_agent .wpsight-listing-agent")[2]
