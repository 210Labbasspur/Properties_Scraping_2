...
class Buckingham_Sale(scrapy.Spider):
    name = "Buckingham_Sale"

    url = "https://www.buckinghamandcompany.com.au/properties-for-sale/"
    prefix = "https://www.buckinghamandcompany.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Buckingham_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2532)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Buckingham and Company'
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".jds-description-container p ::text").getall())  # description
            ...
            if response.css(".epl-author-box-outer-wrapper"):
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.css('.epl-sd-entry-email ::text').get('').strip()
                    item['Field20'] = agent1.css('.epl-sd-entry-mobile ::text').get('').strip()
                    item['Field21'] = agent1.css('.epl-sd-entry-office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.css('.epl-sd-entry-email ::text').get('').strip()
                    item['Field26'] = agent2.css('.epl-sd-entry-mobile ::text').get('').strip()
                    item['Field26A'] = agent2.css('.epl-sd-entry-office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".epl-author-box-outer-wrapper").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field30'] = agent3.css('.epl-sd-entry-email ::text').get('').strip()
                    item['Field31'] = agent3.css('.epl-sd-entry-mobile ::text').get('').strip()
                    item['Field31A'] = agent3.css('.epl-sd-entry-office-phone ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
