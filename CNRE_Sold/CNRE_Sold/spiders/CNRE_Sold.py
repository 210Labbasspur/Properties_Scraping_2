...
class CNRE_Sold(scrapy.Spider):
    name = "CNRE_Sold"
    url = 'https://cnre.com.au/wp-json/agentbox/search?marketingStatus=Sold&page={}'
    prefix = "https://cnre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CNRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        links = []
        for each_url in data['listings']:
            # each_url = each_url.get('webLink')
            id = each_url.get('id')
            each_url = f'https://cnre.agentboxcrm.com.au/{id}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2697)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cooper Newman Real Estate'

            property_street = property.get('property',{}).get('address',{}).get('streetAddress','')
            property_suburb = property.get('property',{}).get('address',{}).get('suburb','')
            state = property.get('property',{}).get('address',{}).get('state','')
            postcode = property.get('property',{}).get('address',{}).get('postcode','')
            country = property.get('property',{}).get('address',{}).get('country','')
            property_address = f"{property_street} {property_suburb} {state} {postcode} {country}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = property.get('property',{}).get('bedrooms','')
            item['Field7'] = property.get('property',{}).get('bathrooms','')
            item['Field8'] = property.get('property',{}).get('totalParking','')
            item['Field9'] = property.get('displayPrice','').replace('Sold','').replace('$','').replace(',','').replace(' ','').replace('-','').replace(':','')
            ...
            item['Field35'] = property.get('property',{}).get('category','')
            item['Field36'] = property.get('property',{}).get('landArea',{}).get('value','')
            item['Field37'] = property.get('property',{}).get('buildingArea',{}).get('value','')

            feature_count = 58
            for feature in response.xpath("//*[contains(@class,'col-md-6 col-sm-6 col-xs-6')]"):   ##  first 4 features are ID, bed, bath etc.
                if 'condition' in ''.join(feature.css('::text').getall()).lower():
                    item['Field52'] = ''.join(feature.css('::text').getall()).replace('\n','').replace('\t','').replace('\r','')
                else:
                    item[f'Field{feature_count}'] = ''.join(feature.css('::text').getall()).replace('\n','').replace('\t','').replace('\r','')
                    feature_count += 1
