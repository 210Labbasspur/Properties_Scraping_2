...
class RichardsonRE_Sold(scrapy.Spider):
    name = "RichardsonRE_Sold"

    url = "https://hfrcolac.com.au/sold"
    prefix = "https://hfrcolac.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(RichardsonRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://hfrcolac.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2835)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Richardson Real Estate'
            ...
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-bath')]/parent::*[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-car-side')]/parent::*[1]/text()").getall())

            price = response.css('.pp_info_header_label_blck ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            img_js = response.xpath("//*[contains(text(),'image_dimensions')]/text()").get('').strip()
            prop_images = re.findall(r'"url":"(https://resize\.lockedoncloud\.com[^"]+\.jpg)"', img_js)
            images = []
            ...
            if response.css(".pp_info_agent_blck"):
                ...
                agent1_name = agent1.css('div>div:nth-child(1) ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'fa-at')]/parent::*/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-mobile')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    agent_image = re.search(r'url\((.*?)\)', agent_image).group(1)
                    images_string = f'https://hfrcolac.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".pp_agent_blck").getall()) > 1:
                ...
                agent2_name = agent2.css('div div div:nth-child(1) ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    # item['Field24'] = agent2.css('div div div:nth-child(2) ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'fa-at')]/parent::*/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-mobile')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".pp_agent_blck").getall()) > 2:
                ...
                agent3_name = agent3.css('div div div:nth-child(1) ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    # item['Field29'] = agent3.css('div div div:nth-child(2) ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'fa-at')]/parent::*/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-mobile')]/parent::*/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
