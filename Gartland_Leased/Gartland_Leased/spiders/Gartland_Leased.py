...
class Gartland_Leased(scrapy.Spider):
    name = "Gartland_Leased"

    url = "https://www.gartland.com.au/leased/"
    prefix = "https://www.gartland.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Gartland_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-feature a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.gartland.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2520)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Gartland'

            bbc = response.css('.single-property__details')
            property_street = bbc.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in bbc.css('.property__address .address ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = bbc.xpath(".//*[contains(@d,'M25.2566 9.21898V8.54758H25.245V7')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = bbc.xpath(".//*[contains(@d,'M12.319 5.07434C11.3503 ')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = bbc.xpath(".//*[contains(@d,'M9.29636 11.2987H16.4886V10')]/parent::*[1]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = ''.join(e.strip() for e in bbc.css('.price ::text').getall() if e.strip()).replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.single-property__copy ::text').getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".single-property__mobile-agents-accounts .property-agents__agent"):
                agent1 = response.css(".single-property__mobile-agents-accounts .property-agents__agent")[0]
            ...
            if len(response.css(".single-property__mobile-agents-accounts .property-agents__agent").getall()) > 1:
                agent2 = response.css(".single-property__mobile-agents-accounts .property-agents__agent")[1]
            ...
            if len(response.css(".single-property__mobile-agents-accounts .property-agents__agent").getall()) > 2:
                agent3 = response.css(".single-property__mobile-agents-accounts .property-agents__agent")[2]
