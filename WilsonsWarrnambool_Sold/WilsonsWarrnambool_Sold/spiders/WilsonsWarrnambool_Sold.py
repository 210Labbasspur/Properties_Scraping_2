...
class WilsonsWarrnambool_Sold(scrapy.Spider):
    name = "WilsonsWarrnambool_Sold"

    url = "https://wilsonswarrnambool.com.au/selling/recently-sold/?pagination_id=1&instance_id=1"
    prefix = "https://wilsonswarrnambool.com.au"
    headers = {
    ...
    count = 1
    db = 'WilsonsWarrnambool'
    collection = 'WilsonsWarrnambool_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(WilsonsWarrnambool_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-listing-post ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2674)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wilsons Warrnambool & District Real Estate'

            property_street = response.css('.property-address-banner .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-address-banner .entry-title-sub span ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.css(".mobileonly .beds span ::text").get('').strip()
            item['Field7'] = response.css(".mobileonly .bath span ::text").get('').strip()
            item['Field8'] = response.css(".mobileonly .parking span ::text").get('').strip()

            price = response.css('.property-address-banner .page-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.propleft .entry-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".propleft .entry-content ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'et_pb_gallery_image')]//a/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".et_pb_section_3"):
                agent1 = response.css(".et_pb_section_3")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('p span:nth-child(3) ::text').get('').strip()
            ...
            if len(response.css(".et_pb_section_3").getall()) > 1:
                agent2 = response.css(".et_pb_section_3")[1]
                agent2_name = agent2.css('.name a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('p span:nth-child(3) ::text').get('').strip()
            ...
            if len(response.css(".et_pb_section_3").getall()) > 2:
                agent3 = response.css(".et_pb_section_3")[2]
                agent3_name = agent3.css('.name a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('p span:nth-child(3) ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("imf ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.css(".propleft .land .icon-value ::text").get('').strip()
            if land_area:
