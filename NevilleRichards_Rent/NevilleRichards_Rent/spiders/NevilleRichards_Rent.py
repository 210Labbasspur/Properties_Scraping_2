...
class NevilleRichards_Rent(scrapy.Spider):
    name = 'NevilleRichards_Rent'
    prefix = 'https://www.nevillerichards.com.au'
    url = "https://www.nevillerichards.com.au/renting/properties-for-lease/"
    headers = {
    ...
    count = 1
    db = 'NevilleRichards'
    collection = 'NevilleRichards_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(NevilleRichards_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2622)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Neville Richards'
            ...
            carport = int(response.xpath("//*[contains(text(),'Carport')]/following-sibling::div[1]/text()").get('').strip()) if response.xpath("//*[contains(text(),'Garage')]/following-sibling::div[1]/text()").get('').strip() else 0
            item['Field8'] = int(carspace) + int(garage)+ int(carport)
