...
class AceRE_Sale(scrapy.Spider):
    name = "AceRE_Sale"

    url = "https://www.acerealestate.com.au/property-status/for-sale/"
    prefix = "https://www.acerealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AceRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".pt-lt-img a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2545)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Ace Real Estate'

            property_street = response.css('.dt-dec-title h2 ::text').get('').strip()  # property_street
            ...
            bbc = response.css(".property-tagsr")
            item['Field6'] = bbc.xpath(".//*[contains(@class,'icon-bed')]/parent::span[1]/text()").get('').strip()
            item['Field7'] = bbc.xpath(".//*[contains(@class,'icon-bath')]/parent::span[1]/text()").get('').strip()
            item['Field8'] = bbc.xpath(".//*[contains(@class,'icon-garage')]/parent::span[1]/text()").get('').strip()
            ...
            prop_images = response.xpath("//*[contains(@alt,'Image Slider')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-detail .left-box"):
                agent1 = response.css(".agent-detail .left-box")[0]
                agent1_name = agent1.css('h3 ::text').get('').strip().replace('Agent ','')
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('h3 ::text').get('').strip().replace('Agent ','')
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.css(".mobile ::text").get('').strip().replace('Mobile :','')
                    item['Field21'] = agent1.css(".office ::text").get('').strip().replace('Office :','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-detail .left-box").getall()) > 1:
                agent2 = response.css(".agent-detail .left-box")[1]
                agent2_name = agent2.css('h3 ::text').get('').strip().replace('Agent ','')
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('h3 ::text').get('').strip().replace('Agent ','')
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.css(".mobile ::text").get('').strip().replace('Mobile :','')
                    item['Field26A'] = agent2.css(".office ::text").get('').strip().replace('Office :','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-detail .left-box").getall()) > 2:
                agent3 = response.css(".agent-detail .left-box")[2]
                agent3_name = agent3.css('h3 ::text').get('').strip().replace('Agent ','')
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('h3s ::text').get('').strip().replace('Agent ','')
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.css(".mobile ::text").get('').strip().replace('Mobile :','')
                    item['Field31A'] = agent3.css(".office ::text").get('').strip().replace('Office :','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field35'] = response.css('.property-col-1 small ::text').get('').strip().replace('-','')
            ...
            #
            feature_count = 58
            for feature in response.css(".additional-details li strong")[:-1]:   ##  first 4 features are ID, bed, bath etc.
