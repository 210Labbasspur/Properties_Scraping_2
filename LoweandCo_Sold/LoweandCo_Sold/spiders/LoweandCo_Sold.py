...
class LoweandCo_Sold(scrapy.Spider):
    name = "LoweandCo_Sold"

    url = "https://www.loweandco.nz/recently-sold/"
    prefix = "https://www.loweandco.nz"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LoweandCo_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-image ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.loweandco.nz{each_url}'
        ...
        next_page = response.css(".right ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2702)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Lowe & Co'

            property_street = response.xpath("//*[text()='Address']/following-sibling::*[1]/text()").get('').strip()  # property_street
            ...
            item['Field8'] = response.xpath("//*[contains(@class,'icon car')]/span[1]/text()").get('').strip()

            item['Field9'] = response.xpath("//*[text()='Price']/following-sibling::*[1]/text()").get('').strip()  # property_street

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.left-side .general-heading ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".left-side .listing-description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".property-thumbnail img ::attr(src)").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('property-slide-thumb','full').replace(");","")
            ...
            '''          AGENTS          '''
            if response.css(".listing-agent-card"):
                agent1 = response.css(".listing-agent-card")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.job-title ::text').get('').strip()
            ...
            if len(response.css(".listing-agent-card").getall()) > 1:
                agent2 = response.css(".listing-agent-card")[1]
                agent2_name = agent2.css('h3 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.job-title ::text').get('').strip()
            ...
            if len(response.css(".listing-agent-card").getall()) > 2:
                agent3 = response.css(".listing-agent-card")[2]
                agent3_name = agent3.css('h3 a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.job-title ::text').get('').strip()
            ...
            item['Field33'] = response.xpath("//*[text()='Listing ID']/following-sibling::*[1]/text()").get('').strip()  # property_street
            item['Field35'] = response.xpath("//*[text()='Type']/following-sibling::*[1]/text()").get('').strip()  # property_street

            land_area = response.xpath("//*[text()='Land Area']/following-sibling::*[1]/text()").get('').strip()  # property_street
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m&sup2','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[text()='Floor Area']/following-sibling::*[1]/text()").get('').strip()  # property_street
            if floor_area:
                if 'Acres' in floor_area:
                    pass
                else:
                    item['Field37'] = floor_area.replace('m&sup2','').replace('m²','')   # land_area
