...
class GreatOceanRoad_Rent(scrapy.Spider):
    name = "GreatOceanRoad_Rent"

    url = "https://www.greatoceanroadrealestate.com.au/leasing/"
    prefix = "https://www.greatoceanroadrealestate.com.au"
    headers = {
    ...
    count = 1
    db = 'GreatOceanRoad'
    collection = 'GreatOceanRoad_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(GreatOceanRoad_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2513)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Great Ocean Road Real Estate'

            property_street = response.css('.address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.font-size-30 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            bbc = response.css('.description')
            item['Field6'] = bbc.xpath("//*[contains(@class,'icon-bedroom')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = bbc.xpath("//*[contains(@class,'icon-bathroom')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = bbc.xpath("//*[contains(@class,'icon-carspace')]/parent::*[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','').replace('Undisclosed','')
            ...
            if response.css("div.agent"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".fit-height ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css("div.agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".fit-height ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css("div.agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".fit-height ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
