...
class McNAMARA_Sold(scrapy.Spider):
    name = "McNAMARA_Sold"

    url = "https://www.mcnamararealestate.com.au/search/?show_in_sale=true&sold=1&pageno={}"
    prefix = "https://www.mcnamararealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(McNAMARA_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("#properties .property>a ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//li[@class='active']/following-sibling::li[1]/a/@href").get('').strip()
        if next_page:
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2517)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'McNAMARA Real Estate'

            property_street = response.css('.property-title h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-title figure ::text').getall() if e.strip())  # property_suburb
            ...
            bbc = response.css('#quick-summary')
            item['Field6'] = bbc.xpath(".//*[contains(text(),'Beds:')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = bbc.xpath(".//*[contains(text(),'Baths:')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = bbc.xpath(".//*[contains(text(),'Parking:')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('span.price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#description p ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'image-popup')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-info"):
                agent1 = response.css(".agent-info")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//dd/text()").get('').strip().replace('tel:','')
                    phone_nos = agent1.xpath(".//dd/text()").getall()
                    if len(phone_nos) > 1:
            ...
            if len(response.css(".agent-info").getall()) > 1:
                agent2 = response.css(".agent-info")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//dd/text()").get('').strip().replace('tel:','')
                    phone_nos = agent2.xpath(".//dd/text()").getall()
                    if len(phone_nos) > 1:
            ...
            if len(response.css(".agent-info").getall()) > 2:
                agent3 = response.css(".agent-info")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//dd/text()").get('').strip().replace('tel:','')
                    phone_nos = agent3.xpath(".//dd/text()").getall()
                    if len(phone_nos) > 1:
            ...
            item['Field35'] = bbc.xpath(".//*[contains(text(),'Property Type:')]/following-sibling::*[1]/text()").get('').strip()

            land_area = bbc.xpath(".//*[contains(text(),'Area:')]/following-sibling::*[1]/text()").get('').strip().lower().replace('m2','')
            if land_area:
            ...
            feature_count = 58
            for feature in response.css(".property-features-list li"):   ##  first 4 features are ID, bed, bath etc.
