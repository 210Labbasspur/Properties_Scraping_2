...
class AbodePeninsula_Rent(scrapy.Spider):
    name = "AbodePeninsula_Rent"

    url = "https://www.abodepeninsula.com.au/properties-for-lease"
    prefix = "https://www.abodepeninsula.com.au"
    headers = {
    ...
    count = 1
    db = 'AbodePeninsula'
    collection = 'AbodePeninsula_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(AbodePeninsula_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2692)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Abode Peninsula'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'bedroom')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(text(),'bathroom')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(text(),'parking')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.xpath("//*[@class='propertyPage-property-contracttext']/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.user-content .propertyPage-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".user-content .user-content ::text").getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@alt,'Photo ')]/@src").getall()
            images = []
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    images_string = f'https://www.abodepeninsula.com.au{agent_image}' if agent_image else ""
            ...
            feature_count = 58
            for feature in response.xpath("//*[contains(text(),'Property Features')]/following-sibling::ul/li/span[1]"):   ##  first 4 features are ID, bed, bath etc.
