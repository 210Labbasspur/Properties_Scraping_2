...
class PaganRE_Sold(scrapy.Spider):
    name = "PaganRE_Sold"

    url = "https://www.paganrealestate.com.au/data/results/?listing_sale_method=Sale&listing_status=sold&status=sold&pg={}"
    prefix = "https://www.paganrealestate.com.au"
    headers = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
    'priority': 'u=0, i',
    'referer': 'https://www.paganrealestate.com.au/sold',
    ...
    def __init__(self, *args, **kwargs):
        super(PaganRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.paganrealestate.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2694)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Pagan Real Estate'

            property_street = response.css('.desktop-only h2 span ::text').get('').strip()  # property_street
            ...
            bbc = ' '.join(e.strip().replace('\n','').replace('\t','') for e in response.css('.mobile-only .bbc ::text').getall() if e.strip())
            m = re.findall(r'(\d+)\s+(Bed|Bath|Car)', bbc)
            data = dict((label, num) for num, label in m)
            item['Field6'] = data.get('Bed', '')
            item['Field7'] = data.get('Bath', '')
            item['Field8'] = data.get('Car', '')

            price = response.xpath("//*[contains(text(),'Property Type')]/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.mobile-only .description ::text').getall())  # description
