...
class LeadersEA_Sold(scrapy.Spider):
    name = "LeadersEA_Sold"
    url = 'https://www.leadersestateagents.com/wp-admin/admin-ajax.php'
    prefix = "https://www.leadersestateagents.com"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LeadersEA_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2575)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Leaders Estate Agents'
            ...
            prop_images = response.css(".img-orientation-landscape ::attr(src)").getall()
            images = []
