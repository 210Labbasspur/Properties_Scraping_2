...
class SkadRE_Sale(scrapy.Spider):
    name = "SkadRE_Sale"

    url = "https://skadrealestate.com.au/properties-for-sale/"
    prefix = "https://skadrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(SkadRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2518)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'SKAD Real Estate'

            property_street = response.css('.page-link .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.page-link .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bedrooms','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/parent::*[1]/text()").get('').strip().lower().replace('bath','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/parent::*[1]/text()").get('').strip().lower().replace('garage','').replace('parking','').replace('s','')
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.overview-content ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[@u='image']/@href").getall()
            prop_images.append(response.xpath("//*[@property='og:image']/@content").get('').strip())
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]"):
                agent1 = response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]")[0]
                agent1_name = agent1.css('.underline ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]").getall()) > 1:
                agent2 = response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]")[1]
                agent2_name = agent2.css('.underline ::text').get('').strip()
                if agent2:
            ...
            if len(response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]").getall()) > 2:
                agent3 = response.xpath("//*[@class='agent-chat-info']/ancestor::div[@class='row'][1]")[2]
                agent3_name = agent3.css('.underline ::text').get('').strip()
                if agent3:
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID :')]/*[1]/text()").get('').strip()
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type')]/*[1]/text()").get('').strip()

            # item['Field40'] = 'video'
            # item['Field41'] = 'floorplan'
            # item['Field42'] = 'virtualwalkthrough'
            # item['Field80'] = 'Contract'
            item['Field81'] = response.xpath("//*[contains(text(),'Inspection Times')]/*[1]/text()").get('').strip()

            land_area = response.xpath("//*[contains(text(),'Area')]/*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('squareMeter','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
