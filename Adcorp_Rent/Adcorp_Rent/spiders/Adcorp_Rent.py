...
class Adcorp_Rent(scrapy.Spider):
    name = 'Adcorp_Rent'
    url = "https://www.adcorpgroup.com.au/renting/for-lease/"
    prefix = 'https://www.adcorpgroup.com.au'
    headers = {
    ...
    count = 1
    db = 'Adcorp'
    collection = 'Adcorp_Rent'
    # bucket_prefix = f'D_{collection}'
    bucket_prefix = f'Images10'
    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Adcorp_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2387)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Adcorp Property Group'
            ...
            item['Field12'] = ' '.join(element.strip().replace('\t', ' ').replace('\n', ' ').replace('\r', ' ') for element in description)  # description
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Listing ID')]/following-sibling::*[1]/text()").get('').strip().replace('Property ID:','')
