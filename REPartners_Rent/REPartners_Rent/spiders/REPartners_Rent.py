...
class REPartners_Rent(scrapy.Spider):
    name = "REPartners_Rent"

    url = "https://www.realestatepartners.net.au/type/rental/"
    prefix = "https://www.realestatepartners.net.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(REPartners_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".title-wrapper ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2461)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Real Estate Partners'
            ...
            price = response.xpath("//*[text()='Price: ']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.duo-property-desc ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'slide_element')]//a/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-contact-details"):
                agent1 = response.css(".agent-contact-details")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(@alt,'{agent1_name}')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-contact-details").getall()) > 1:
                agent2 = response.css(".agent-contact-details")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(@alt,'{agent2_name}')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-contact-details").getall()) > 2:
                agent3 = response.css(".agent-contact-details")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = response.xpath(f"//*[contains(@alt,'{agent3_name}')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'httpspacepacomau{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath("//*[text()='Type: ']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
