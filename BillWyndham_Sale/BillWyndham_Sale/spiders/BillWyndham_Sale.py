...
class BillWyndham_Sale(scrapy.Spider):
    name = "BillWyndham_Sale"

    url = "https://www.billwyndham.com.au/listings/sold"
    prefix = "https://www.billwyndham.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(BillWyndham_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.billwyndham.com.au{each_url}'
        ...
        next_page = response.css("a.cursor-pointer ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2499)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Bill Wyndham & CO Real Estate'

            property_street = response.css('.address-text ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'Bedrooms')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'Bathrooms')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'Parking')]/span[1]/text()").get('').strip()

            price = response.xpath("//*[@aria-label='price']/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.address-headline-wrap .headline ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[@class='mt-7 cms-text']//p/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'w-auto h-full')]//img/@alt").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".form-agent-card"):
                agent1 = response.css(".form-agent-card")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.position-area-wrap span ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".w-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".form-agent-card").getall()) > 1:
                agent2 = response.css(".form-agent-card")[1]
                agent2_name = agent2.css('.agent-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.position-area-wrap span ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".w-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".form-agent-card").getall()) > 2:
                agent3 = response.css(".form-agent-card")[2]
                agent3_name = agent3.css('.agent-name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.position-area-wrap span ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".w-full ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.css('.border-r-c-sky-1 .italic ::text').get('').strip()
