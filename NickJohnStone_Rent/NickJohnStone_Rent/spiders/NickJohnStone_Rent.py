...
class NickJohnStone_Rent(scrapy.Spider):
    name = 'NickJohnStone_Rent'
    url = "https://www.nickjohnstone.com.au/propertiesJson?callback=angular.callbacks._1&currentPage={}&perPage=12&sort=d_listing%20desc&listing_cat=rental"
    prefix = 'https://www.nickjohnstone.com.au'
    headers = {
        'accept': '*/*',
    ...
    count = 1
    db = 'NickJohnStone'
    collection = 'NickJohnStone_Rent'
    bucket_prefix = f'D_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(NickJohnStone_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def Detail_parse(self, mini_data,full_data):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2538)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Nick John Stone'
