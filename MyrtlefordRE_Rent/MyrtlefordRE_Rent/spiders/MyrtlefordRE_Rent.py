...
class MyrtlefordRE_Rent(scrapy.Spider):
    name = "MyrtlefordRE_Rent"

    url = "https://www.myrtlefordrel.com.au/rent/residential-rentals/"
    prefix = "https://www.myrtlefordrel.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MyrtlefordRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2648)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Myrtleford Real Estate & Livestock'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'la-bed')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'la-bath')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'la-car')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            prop_images = response.xpath("//source[contains(@srcset,'https')]/@srcset").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.strip().split(',')[-1].split()[0]
            ...
            land_area = response.xpath("//*[contains(@class,'property-land-size')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land size:','').replace('sqm','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
