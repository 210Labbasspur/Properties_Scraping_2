...
class Kardinia_Sold(scrapy.Spider):
    name = "Kardinia_Sold"

    url = "https://www.kardiniaproperty.com.au/recently-sold-properties/"
    prefix = "https://www.kardiniaproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Kardinia_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".view-listing ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2552)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Kardinia Property'

            property_street = response.css('.property-street-address ::text').get('').strip()  # property_street
            property_suburb = response.css('.property-postcode-city ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'beds-dark')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'baths-dark')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'cars-dark')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[contains(text(),'Price guide')]/following-sibling::*[1]/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ''.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#single-property-main>h2 ::text').getall()[0])  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#single-property-main>p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'carousel-item')]/@data-lazy").getall()
            images = []
            ...
            land_area = response.xpath("//*[contains(@src,'area-dark')]/preceding-sibling::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m2','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
