...
class HaymanPartners_Rent(scrapy.Spider):
    name = "HaymanPartners_Rent"

    url = "https://haymanpartners.com.au/buy/?property-type=rent&suburbs=&sortBy=new-old&price-range--min=50.00&price-range--max=3000.00&bed_count__min=0&bed_count__max=4&bath_count__min=0&bath_count__max=2&garage_count__min=0&garage_count__max=2"
    prefix = "https://haymanpartners.com.au"
    headers = {
    ...
    count = 1
    db = 'HaymanPartners'
    collection = 'HaymanPartners_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(HaymanPartners_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".lower ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2422)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Hayman Partners'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.grid h1 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css("#features .bed ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in response.css("#features .bath ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in response.css("#features .car ::text").getall())
            ...
            prop_images = response.xpath("//*[contains(@class,'swiper-slide')]/img/@data-lazy-src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-card-box"):
                agent1 = response.css(".agent-card-box")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".img-wrap ::attr(data-bg)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-card-box").getall()) > 1:
                agent2 = response.css(".agent-card-box")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".img-wrap ::attr(data-bg)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-card-box").getall()) > 2:
                agent3 = response.css(".agent-card-box")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".img-wrap ::attr(data-bg)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(text(),'Land size')]/*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Building size")]/*[1]/text()').get('').strip()
            if floor_area:
