...
class Vivid_Rent(scrapy.Spider):
    name = "Vivid_Rent"

    url = "https://www.vivid.com.au/for-rent/"
    prefix = "https://www.vivid.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Vivid_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2479)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Vivid Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'Beds')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@title,'Baths')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@title,'Car Park')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            land_area = response.xpath("//*[contains(@title,'Land Area')]/*[2]/text()").get('').strip().replace('m2','')
            if land_area:
