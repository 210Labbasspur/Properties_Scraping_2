...
class GagliardiScott_Sale(scrapy.Spider):
    name = "GagliardiScott_Sale"

    url = "https://www.gagliardiscott.com.au/search/?show_in_sale=true&sold=0&show_in_residential=true"
    prefix = "https://www.gagliardiscott.com.au"
    headers = {
    ...
    count = 1
    db = 'GagliardiScott'
    collection = 'GagliardiScott_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(GagliardiScott_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        next_page = response.xpath("//*[contains(@class,'fa fa-chevron-right')]/parent::a[1]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2757)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Gagliardi Scott Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('h1.light ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field7'] = response.xpath("//*[contains(@class,'fa-shower')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.property-header-attributes .property-attr ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-item-header h2 ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'property-gallery-image')]/@href").getall()
            images = []
            ...
            if response.css(".agent-list"):
                ...
                agent1_name = agent1.css('.agent-card-title a ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = ''.join(e.strip() for e in agent1.xpath(".//*[text()='mobile']/parent::*/text()").getall())
                    item['Field21'] = ''.join(e.strip() for e in agent1.xpath(".//*[text()='office']/parent::*/text()").getall())
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-list-image ::attr(style)").get('').strip().replace("background-image: url('",'').replace("');",'')
            ...
            if len(response.css(".agent-list").getall()) > 1:
                ...
                agent2_name = agent2.css('.agent-card-title a ::text').get('').strip()
                if agent2:
                    ...
                    item['Field26'] = ''.join(e.strip() for e in agent2.xpath(".//*[text()='mobile']/parent::*/text()").getall())
                    item['Field26A'] = ''.join(e.strip() for e in agent2.xpath(".//*[text()='office']/parent::*/text()").getall())
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-list-image ::attr(style)").get('').strip().replace("background-image: url('",'').replace("');",'')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-list").getall()) > 2:
                ...
                agent3_name = agent3.css('.agent-card-title a ::text').get('').strip()
                if agent3:
                    ...
                    item['Field31'] = ''.join(e.strip() for e in agent3.xpath(".//*[text()='mobile']/parent::*/text()").getall())
                    item['Field31A'] = ''.join(e.strip() for e in agent3.xpath(".//*[text()='office']/parent::*/text()").getall())
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-list-image ::attr(style)").get('').strip().replace("background-image: url('",'').replace("');",'')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land = response.xpath("//*[contains(@class,'fa-expand')]/parent::*[1]/text()").get('').strip()
