...
class Doepels_Rent(scrapy.Spider):
    name = "Doepels_Rent"

    url = "https://www.doepels.com.au/renting/properties-for-rent/"
    prefix = "https://www.doepels.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Doepels_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2627)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Doepel Lilley + Taylor Real Estate'

            property_street = response.xpath("//*[contains(text(),'Location')]/following-sibling::*[1]/text()").get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/following-sibling::*[1]/text()").get('').strip()  # price
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms')]/following-sibling::*[1]/text()").get('').strip()  # price
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/following-sibling::*[1]/text()").get('').strip()  # price

            price = response.xpath("//*[contains(text(),'Price')]/following-sibling::*[1]/text()").get('').strip()  # price
            ...
            prop_images = response.xpath("//*[contains(@class,'image-gallery-item')]/@href").getall()
            images = []
            ...
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'Property ID')]/following-sibling::*[1]/text()").get('').strip()  # Type
            item['Field35'] = response.xpath("//*[contains(text(),'Type')]/following-sibling::*[1]/text()").get('').strip()  # Type

            land = response.xpath("//*[contains(text(),'Land Size')]/following-sibling::*[1]/text()").get('').strip()  # Type
            ...
            feature_count = 58
            for feature in response.css(".detail-feature li"):   ##  first 4 features are ID, bed, bath etc.
