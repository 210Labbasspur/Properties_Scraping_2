...
class Klemich_Rent(scrapy.Spider):
    name = "Klemich_Rent"

    url = "https://klemich.com.au/rental-properties"
    prefix = "https://klemich.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Klemich_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-article__link ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'pagination-item  pagination-item--active')]/following-sibling::*[1]//a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2432)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Klemich Real Estate'

            property_street = response.css('.h2--large ::text').get('').strip()  # property_street
            ...
            bbc = response.css('.image-text__text__inner')
            # item['Field6'] = response.xpath("//img[@alt='Bedroom']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field6'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(text(),'Bedrooms:')]/parent::*[1]/text()").getall()).replace('Bedrooms:','').replace('bed','')
            item['Field7'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(text(),'Bathrooms:')]/parent::*[1]/text()").getall()).replace('Bathrooms:','').replace('bath','')
            item['Field8'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(text(),'Car spaces:')]/parent::*[1]/text()").getall()).replace('Car spaces:','').replace('car','')

            price = ''.join(e.strip() for e in bbc.xpath(".//*[contains(text(),'Price:')]/parent::*[1]/text()").getall()).replace('Price:','').replace('car','').replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-banner__floorplans p ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-details__details ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@data-imagelightbox,'photos')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".profile-article__inner"):
                agent1 = response.css(".profile-article__inner")[0]
                agent1_name = agent1.css('.profile-article__title ::text').get('').strip()
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".profile-article__image__img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".profile-article__inner").getall()) > 1:
                agent2 = response.css(".profile-article__inner")[1]
                agent2_name = agent2.css('.profile-article__title ::text').get('').strip()
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".profile-article__image__img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".profile-article__inner").getall()) > 2:
                agent3 = response.css(".profile-article__inner")[2]
                agent3_name = agent3.css('.profile-article__title ::text').get('').strip()
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".profile-article__image__img ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
