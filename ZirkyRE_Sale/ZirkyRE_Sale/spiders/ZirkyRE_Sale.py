...
class ZirkyRE_Sale(scrapy.Spider):
    name = "ZirkyRE_Sale"

    url = "https://zirkyrealestate.com.au/properties-for-sale"
    prefix = "https://zirkyrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ZirkyRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyTile-body ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2600)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Zirky Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('span.address ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[@class='icon-bed']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = response.xpath("//*[@class='icon-bath']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = response.xpath("//*[@class='icon-car']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.property-header .muted ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.container h2.text-center ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@itemprop,'url')]/@href").getall()
            images = []
            for img in prop_images[:-2]:        ## Scrape maximum 25 photos
            ...
            '''          AGENTS          '''
            if response.css(".offset-pair"):
                agent1 = response.css(".offset-pair")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".bg-cover ::attr(style)").get('').strip().replace("background-image : url('","").replace("')","")
                    images_string = f'https://zirkyrealestate.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".offset-pair").getall()) > 1:
                agent2 = response.css(".offset-pair")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".bg-cover ::attr(style)").get('').strip().replace("background-image : url('","").replace("')","")
                    images_string = f'https://zirkyrealestate.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".offset-pair").getall()) > 2:
                agent3 = response.css(".offset-pair")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".bg-cover ::attr(style)").get('').strip().replace("background-image : url('","").replace("')","")
                    images_string = f'https://zirkyrealestate.com.au{agent_image}' if agent_image else ""
            ...
            #
            feature_count = 58
            for feature_css in response.xpath("//*[contains(text(),'Property Features')]/following-sibling::ul/li"):   ##  first 4 features are ID, bed, bath etc.
                feature = ' '.join(e.strip() for e in feature_css.css('::text').getall())
