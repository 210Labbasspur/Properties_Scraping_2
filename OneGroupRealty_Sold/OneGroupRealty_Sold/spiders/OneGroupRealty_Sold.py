...
class OneGroupRealty_Sold(scrapy.Spider):
    name = "OneGroupRealty_Sold"

    url = "https://www.onegrouprealty.com.au/properties/sold/"
    prefix = "https://www.onegrouprealty.com.au"
    headers = {
    ...
    count = 1
    db = 'OneGroupRealty'
    collection = 'OneGroupRealty_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(OneGroupRealty_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".stretched-link ::attr(href)").getall()
        links = []
        ...
        next_page = response.css(".next-button ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2790)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'One Group Realty'

            property_street = response.css('.col h1.h2 ::text').get('').strip()  # property_street
            ...
            bbc = response.css('.align-self-end')
            item['Field6'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(@class,'icon-bed')]/parent::*[1]/text()").getall() if e.strip())
            item['Field7'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(@class,'icon-bath')]/parent::*[1]/text()").getall() if e.strip())
            item['Field8'] = ''.join(e.strip() for e in bbc.xpath(".//*[contains(@class,'icon-car')]/parent::*[1]/text()").getall() if e.strip())
            ...
            if response.css(".agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'icon-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'icon-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'icon-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath("//*[contains(text(),'Building Size:')]/text()").get('').strip().replace('Building Size:','').replace('m','')
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".features-list li"):   ##  first 4 features are ID, bed, bath etc.
