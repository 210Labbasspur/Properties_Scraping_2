...
class AquaRE_Rent(scrapy.Spider):
    name = "AquaRE_Rent"

    url = "https://www.aquarealestate.com.au/for-lease"
    prefix = "https://www.aquarealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AquaRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".entry-title>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2564)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Aqua Real Estate'

            property_street = response.xpath("//*[contains(@class,'entry-title')]/text()[1]").get('').strip()  # property_street
            ...
            item['Field6'] = response.css(".bed .meta-value ::text").get('').strip().replace('Bedroom','').replace('s','')
            item['Field7'] = response.css(".bath .meta-value ::text").get('').strip().replace('Bathroom','').replace('s','')
            item['Field8'] = response.css(".garage .meta-value ::text").get('').strip().replace('Car Space','').replace('s','')
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.entry-content ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'attachment-listing_large size-listing_large')]/@src").getall()
            images = []
            ...
            if response.css(".agent-card"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(text(),'Mobile:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.xpath(".//*[contains(text(),'Office:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-card").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(text(),'Mobile:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.xpath(".//*[contains(text(),'Office:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-card").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(text(),'Mobile:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.xpath(".//*[contains(text(),'Office:')]/following-sibling::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field35'] = response.xpath("//*[contains(text(),'Property Type')]/following-sibling::*[1]/a/text()").get('').strip()
