...
class GiordanoPartners_Rent(scrapy.Spider):
    name = "GiordanoPartners_Rent"

    url = "https://giordanopartners.com/properties-for-lease"
    prefix = "https://giordanopartners.com"
    headers = {
    ...
    count = 1
    db = 'GiordanoPartners'
    collection = 'GiordanoPartners_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(GiordanoPartners_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2421)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Giordano & Partners Real Estate'

            property_street = response.css('.address>.iblock>span ::text').get('').strip()  # property_street
            property_suburb = response.css('.address>.iblock>span:nth-child(2) ::text').get('').strip()  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.css('.contracttext ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.css(".agentCard"):
                agent1 = response.css(".agentCard")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.agent-title ::text').get('').strip()
            ...
            if len(response.css(".agentCard").getall()) > 1:
                agent2 = response.css(".agentCard")[1]
                agent2_name = agent2.css('.oneline ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent-title ::text').get('').strip()
            ...
            if len(response.css(".agentCard").getall()) > 2:
                agent3 = response.css(".agentCard")[2]
                agent3_name = agent3.css('.oneline ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent-title ::text').get('').strip()
            ...
            land_area = response.css(".text-left-med:nth-child(3) .qty ::text").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.css('.text-left-med:nth-child(3) div:nth-child(2) .qty ::text').get('').strip()
            if floor_area:
