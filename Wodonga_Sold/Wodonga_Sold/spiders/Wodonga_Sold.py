...
class Wodonga_Sold(scrapy.Spider):
    name = "Wodonga_Sold"

    url = "https://www.wodongarealestate.com.au/selling/recent-sales/"
    prefix = "https://www.wodongarealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Wodonga_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2378)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wodonga Real Estate'
            ...
            item['Field6'] = response.xpath("//label[contains(text(),'Bedroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//label[contains(text(),'Bathroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//label[contains(text(),'Parking')]/following-sibling::*[1]/text()").get('').strip()
            ...
            floor_area = response.xpath('//*[contains(text(),"Building Size")]/following-sibling::span[1]/text()').get('').strip()
            if floor_area:
