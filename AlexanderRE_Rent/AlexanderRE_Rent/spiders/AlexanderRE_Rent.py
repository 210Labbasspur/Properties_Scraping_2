...
class AlexanderRE_Rent(scrapy.Spider):
    name = "AlexanderRE_Rent"

    url = "https://www.alexanderrealestate.com.au/rent/"
    prefix = "https://www.alexanderrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AlexanderRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2389)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Alexander Real Estate'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.css("#quickinfo .bedroom ::text").getall() if e.strip())
            item['Field7'] = ''.join(e.strip() for e in response.css("#quickinfo .bathroom ::text").getall() if e.strip())
            item['Field8'] = ''.join(e.strip() for e in response.css("#quickinfo .carspace ::text").getall() if e.strip())
            ...
            prop_images = response.xpath("//*[contains(@class,'img-orientation img-orientation-landscape img-default-used-size')]/@src").getall()
            images = []
            ...
            land_area = ''.join(e.strip() for e in response.css("#quickinfo .land-size ::text").getall() if e.strip())
            if land_area:
