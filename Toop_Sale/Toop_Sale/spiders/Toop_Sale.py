...
class Toop_Sale(scrapy.Spider):
    name = "Toop_Sale"

    url = "https://mcfygsw91e-dsn.algolia.net/1/indexes/*/queries?x-algolia-api-key=edc54929277cbfdb894d1beb36029e4e&x-algolia-application-id=MCFYGSW91E&x-user-agent=Algolia%20for%20vanilla%20JavaScript%203.3.0"
    prefix = "https://www.toop.com.au"
    # data = {"requests":[{"indexName":"prod_SOLD","params":"query=&page=0&hitsPerPage=1000&filters=bedrooms%20%3E%3D%200%20AND%20bathrooms%20%3E%3D%200%20AND%20carports%20%3E%3D%200%20AND%20price%3A%200%20TO%2020000000%20AND%20(listingCategory%3Asale%20OR%20listingCategory%3Avault)%20AND%20(availability%3ASoldPriorToAuction%20OR%20availability%3AVaultSold%20OR%20availability%3Asold)&aroundLatLng=&aroundRadius=5000"}]}
    data = '{"requests":[{"indexName":"prod_LISTINGS","params":"query=&page=0&hitsPerPage=1000&filters=bedrooms%20%3E%3D%200%20AND%20bathrooms%20%3E%3D%200%20AND%20carports%20%3E%3D%200%20AND%20price%3A%200%20TO%2020000000%20AND%20(listingCategory%3Asale%20OR%20listingCategory%3Avault)%20AND%20(availability%3Aundercontract%20OR%20availability%3AAvailable)&aroundLatLng=&aroundRadius=5000"}]}'
    ...
    def __init__(self, *args, **kwargs):
        super(Toop_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        yield scrapy.Request(self.url, method='POST', body=self.data, headers=self.headers, callback=self.parse)
    ...
    def parse(self, response):
        ...
        links = []
        for each_url in data['results'][0]['hits']:
            each_url = each_url.get('slug')
            each_url = f'https://www.toop.com.au/{each_url}'
        ...
        for property in data['results'][0]['hits']:
            slug = property.get('slug')
            each_url = f'https://www.toop.com.au/{slug}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2477)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Toop'

            property_street = property.get('address','')
            property_suburb = property.get('suburb','')
            ...
            if property.get('soldPrice',''):
                item['Field9'] = property.get('soldPrice','')
            else:
                item['Field9'] = property.get('price','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'col-12 offset-sm-1 col-sm-10 offset-lg-1 col-lg-7 offset-xl-2 col-xl-6')]//text()").getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'swiper-slide')]//img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-flex-inner"):
                agent1 = response.css(".agent-flex-inner")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.mb-15 ::text').get('').strip()
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'sp-send-email')]/@data-email").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".agent-flex-inner").getall()) > 1:
                agent2 = response.css(".agent-flex-inner")[1]
                agent2_name = agent2.css('.mb-0 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.mb-15 ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'sp-send-email')]/@data-email").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".agent-flex-inner").getall()) > 2:
                agent3 = response.css(".agent-flex-inner")[2]
                agent3_name = agent3.css('.mb-0 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.mb-15 ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'sp-send-email')]/@data-email").get('').strip().replace('mailto:','')
            ...
            item['Field35'] = property.get('propertyType','')

            land_area = response.xpath("//*[contains(text(),'Land Size')]/parent::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('sqm approx.','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
