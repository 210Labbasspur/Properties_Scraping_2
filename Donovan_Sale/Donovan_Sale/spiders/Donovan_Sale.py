...
class Donovan_Sale(scrapy.Spider):
    name = "Donovan_Sale"

    url = "https://donovanrealestate.com.au/properties-for-sale/"
    prefix = "https://donovanrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Donovan_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2556)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Donovan Real Estate Partners'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,' bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,' bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,' car')]/following-sibling::*[1]/text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.xxxxxxxxxxx ::text').getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".epl-author-author_id"):
                agent1 = response.css(".epl-author-author_id")[0]
            ...
            if len(response.css(".epl-author-author_id").getall()) > 1:
                agent2 = response.css(".epl-author-author_id")[1]
            ...
            if len(response.css(".epl-author-author_id").getall()) > 2:
                agent3 = response.css(".epl-author-author_id")[2]
            ...
            floor_area = response.css('.building-size ::text').get('').strip()
            if floor_area:
