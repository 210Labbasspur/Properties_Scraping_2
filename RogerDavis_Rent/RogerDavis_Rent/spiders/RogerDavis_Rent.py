...
class RogerDavis_Rent(scrapy.Spider):
    name = "RogerDavis_Rent"

    url = "https://www.rogerdavis.com.au/renting/properties-for-lease/"
    prefix = "https://www.rogerdavis.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(RogerDavis_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.rogerdavis.com.au{each_url}'
        ...
        next_page = response.xpath("//*[text()='›']/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2746)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Roger Davis Real Estate'

            property_street = response.css('.address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.address .suburb ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('\xa0',' ')
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'rd-bed')]/parent::*[1]/text()").getall() if e.strip())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'rd-bath')]/parent::*[1]/text()").getall() if e.strip())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'rd-car')]/parent::*[1]/text()").getall() if e.strip())

            price = response.css('.description h5 ::text').get('').strip()
            price = price.split(' ')[-1].replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.description h2 ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'feature-gallery-container')]/img/@data-lazy").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".large-4 p"):
                agent1 = response.css(".large-4 p")[0]
                agent1_name = agent1.css('::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field20'] = agent1.css("em:nth-child(2) ::text").get('').strip()
                    item['Field21'] = agent1.css("em:nth-child(4) ::text").get('').strip()

            if len(response.css(".large-4 p").getall()) > 1:
                agent2 = response.css(".large-4 p")[1]
                agent2_name = agent2.css('::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field26'] = agent2.css("em:nth-child(2) ::text").get('').strip()
                    item['Field26A'] = agent2.css("em:nth-child(4) ::text").get('').strip()

            if len(response.css(".large-4 p").getall()) > 2:
                agent3 = response.css(".large-4 p")[2]
                agent3_name = agent3.css('::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field31'] = agent3.css("em:nth-child(2) ::text").get('').strip()
                    item['Field31A'] = agent3.css("em:nth-child(4) ::text").get('').strip()


            item['Field33'] = re.search(r'/(\d+)/', response.url).group(1)
            # item['Field33'] = response.css('.ididididididid ::text').get('').strip()
            # id_from_url = response.url.split('/')[-1]
