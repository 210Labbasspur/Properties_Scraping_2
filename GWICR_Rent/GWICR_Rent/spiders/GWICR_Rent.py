...
class GWICR_Rent(scrapy.Spider):
    name = "GWICR_Rent"

    url = "https://www.gwicr.com.au/lease/page/{}/"
    prefix = "https://www.gwicr.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GWICR_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".image-fade_in_back a ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[text()='Load more']/text()").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2500)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Gross Waddell ICR'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.medium-7 p.no-margin ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            # NO BBC
            # item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/parent::*[1]/text()").get('').strip()
            # item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/parent::*[1]/text()").get('').strip()
            # item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.row-collapse .small-8 p.no-margin ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.medium-7  div.col-inner p ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'attachment-original')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".portfolio-box"):
                agent1 = response.css(".portfolio-box")[0]
                agent1_name = agent1.css('.heading-font p ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.text:nth-child(2) p ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip().replace('tel:','').replace('%20','-')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(src)").get('').strip()#.replace('200x300','1440x2161').replace(')','')
            ...
            if len(response.css(".portfolio-box").getall()) > 1:
                agent2 = response.css(".portfolio-box")[1]
                agent2_name = agent2.css('.heading-font p ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.text:nth-child(2) p ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip().replace('tel:','').replace('%20','-')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(src)").get('').strip()#.replace('200x300','1440x2161').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".portfolio-box").getall()) > 2:
                agent3 = response.css(".portfolio-box")[2]
                agent3_name = agent3.css('.heading-font p ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.text:nth-child(2) p ::text').get('').strip()
                    ...
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip().replace('tel:','').replace('%20','-')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(src)").get('').strip()#.replace('200x300','1440x2161').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.css('.featured-box:nth-child(1) .icon-box-text p ::text').get('').strip()

            land_area = response.css(".featured-box .icon-box-text p ::text").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('m','').replace('Sqm','').replace(' ','')   # land_area
