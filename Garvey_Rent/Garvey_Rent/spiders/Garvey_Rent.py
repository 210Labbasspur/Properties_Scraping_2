...
class Garvey_Rent(scrapy.Spider):
    name = "Garvey_Rent"

    url = "https://www.garveycompany.com.au/properties-for-lease"
    prefix = "https://www.garveycompany.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Garvey_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".image a:nth-child(1) ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            price = response.css('small.block ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.tiny-12 h2 ::text').getall()[:-1])  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".tiny-12 p ::text").getall()[:-2])  # description
            ...
            prop_images = re.findall(r'/imagecache/bestfit/1280x720/_files/_propertyimages/\S+\.jpg', response.text)
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('background-image:url(','').replace(");","")
                images.append(f'https://www.garveycompany.com.au{img}' if img else "")
            ...
            '''          AGENTS          '''
            if response.css(".thumbItem"):
                agent1 = response.css(".thumbItem")[0]
                agent1_name = agent1.css('.agentName ::text').get('').strip()
                if agent1_name:
                    ...
                    images_string = f'https://www.garveycompany.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.css(".thumbItem").getall()) > 1:
                agent2 = response.css(".thumbItem")[1]
                agent2_name = agent2.css('.agentName ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".thumbItem").getall()) > 2:
                agent3 = response.css(".thumbItem")[2]
                agent3_name = agent3.css('.agentName ::text').get('').strip()
                if agent3:
