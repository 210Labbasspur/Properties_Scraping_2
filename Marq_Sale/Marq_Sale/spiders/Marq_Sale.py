...
class Marq_Sale(scrapy.Spider):
    name = "Marq_Sale"

    url = "https://marqproperty.com.au/buy/properties-youll-love/"
    prefix = "https://marqproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Marq_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".tile-link ::attr(href)").getall()
        links = []
        ...
        next_page = response.css(".text-right .icon-link ::attr(href)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2442)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Marq Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed-front')]/parent::*/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'fa-solid fa-bath')]/parent::*/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'fa-solid fa-car')]/parent::*/preceding-sibling::*[1]/text()").get('').strip()
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]"):
                agent1 = response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]")[0]
                agent1_name = agent1.css('h4.h5 ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]").getall()) > 1:
                agent2 = response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]")[1]
                agent2_name = agent2.css('h4.h5 ::text').get('').strip()
                if agent2:
            ...
            if len(response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]").getall()) > 2:
                agent3 = response.xpath("//*[@class='grid-x grid-padding-x grid-padding-y']//*[contains(@class,'align-middle')]")[2]
                agent3_name = agent3.css('h4.h5 ::text').get('').strip()
                if agent3:
            ...
            land_area = response.xpath("//*[contains(text(),'Land size')]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('size','').replace('m','').replace('sqm','').replace(':','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(text(),'House size')]/text()").get('').strip()
            if floor_area:
                if 'Acres' in floor_area:
                    pass
                else:
                    item['Field37'] = floor_area.replace('House','').replace('size','').replace('m','').replace('sqm','').replace(':','').replace(' ','')   # land_area
