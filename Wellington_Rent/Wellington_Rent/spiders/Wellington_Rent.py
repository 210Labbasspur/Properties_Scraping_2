...
class Wellington_Rent(scrapy.Spider):
    name = "Wellington_Rent"
    url = ('https://wellingtonrealestate.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bcount%5D%5Bvalue%5D=20&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Bmeta_key%5D%5Bvalue%5D=dateListed&query%5Bsold%5D%5Bvalue%5D=0&query%5BsaleOrRental%5D%5Bvalue%5D=Rental&query%5BsaleOrRental%5D%5Btype%5D=equal&query%5Border%5D%5Bvalue%5D=dateListed-desc&query%5Border%5D%5Btype%5D=equal&query%5Bdoing_wp_cron%5D%5Bvalue%5D=1739583391.6830921173095703125000&query%5Bdoing_wp_cron%5D%5Btype%5D=equal&query%5Bextended%5D%5Bvalue%5D=0&'
           'query%5Bpaged%5D%5Bvalue%5D={}')
    prefix = "https://wellingtonrealestate.com.au"
    headers = {
    ...
    count = 1
    db = 'Wellington'
    collection = 'Wellington_Rent'
    bucket_prefix = f'Images6'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Wellington_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2550)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wellington Real Estate'
