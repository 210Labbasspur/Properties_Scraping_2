...
class KeepingItRealty_Rent(scrapy.Spider):
    name = "KeepingItRealty_Rent"

    url = "https://www.keepingitrealty.com.au/properties-for-lease/"
    prefix = "https://www.keepingitrealty.com.au"
    headers = {
    ...
    count = 1
    db = 'KeepingItRealty'
    collection = 'KeepingItRealty_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(KeepingItRealty_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".meta a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3379)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Keeping It Realty'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.et_pb_row_0 .et_pb_text_inner h1 ::text').getall() if e.strip())  # property_suburb
            ...
            bb = response.xpath("//*[contains(text(),'BED')]/text()").get('').strip().lower().replace('bed','')
            # item['Field6'] = response.xpath("//*[contains(text(),'BED')]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('|','')
            item['Field6'] = bb.split('|')[0] if bb else ''
            item['Field7'] = bb.split('|')[-1] if bb else ''
            item['Field8'] = response.xpath("//*[contains(text(),'CAR')]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('|','')

            price = response.xpath("//*[contains(text(),'SALE PRICE')]/text()").get('').strip().replace('SALE PRICE:','').replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
            item['Field11'] = response.xpath("//*[contains(text(),'SETTLEMENT DATE:')]/text()").get('').strip().replace('SETTLEMENT DATE:','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.et_pb_row_2 .et_pb_text_inner ::text').getall())  # description
            ...
            land_area = response.xpath("//*[contains(text(),'Land Area')]/preceding-sibling::*[1]//*[@class='percent-value']/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(text(),'Building Area')]/preceding-sibling::*[1]//*[@class='percent-value']/text()").get('').strip()
            if floor_area:
