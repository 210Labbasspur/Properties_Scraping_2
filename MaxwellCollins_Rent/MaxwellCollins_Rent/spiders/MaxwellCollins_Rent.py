...
class MaxwellCollins_Rent(scrapy.Spider):
    name = "MaxwellCollins_Rent"

    url = "https://www.maxwellcollins.com.au/rent/search-rentals/"
    prefix = "https://www.maxwellcollins.com.au"
    headers = {
    ...
    count = 1
    db = 'MaxwellCollins'
    collection = 'MaxwellCollins_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(MaxwellCollins_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2666)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Maxwell Collins'
            ...
            land_area = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'land-size')]/text()").getall())
            if land_area:
