...
class SGLRealEstate_Sold(scrapy.Spider):
    name = "SGLRealEstate_Sold"

    url = "https://sglrealestate.com.au/sell/"
    prefix = "https://sglrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(SGLRealEstate_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".card>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2805)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Southern Grampians Livestock & Real Estate'

            property_street = response.css('.property-info__address ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@aria-label,'bedroom')]/div/text()").get('').strip().lower().replace('bed','').replace('s','')
            item['Field7'] = response.xpath("//*[contains(@aria-label,'bathroom')]/div/text()").get('').strip().lower().replace('bath','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(@aria-label,'parking space')]/div/text()").get('').strip().lower().replace('car','').replace('s','')

            price = response.css('.property-info__price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
            item['Field11'] = response.css('.property-info__sold ::text').get('').strip().replace('Sold','').replace('on','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@class,'js-lightbox__media-inner')]/img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent__body"):
                agent1 = response.css(".agent__body")[0]
                agent1_name = agent1.css('.agent__title a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.agent__role ::text').get('').strip()
            ...
            if len(response.css(".agent__body").getall()) > 1:
                agent2 = response.css(".agent__body")[1]
                agent2_name = agent2.css('.agent__title a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.agent__role ::text').get('').strip()
            ...
            if len(response.css(".agent__body").getall()) > 2:
                agent3 = response.css(".agent__body")[2]
                agent3_name = agent3.css('.agent__title a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.agent__role ::text').get('').strip()
            ...
            item['Field35'] = response.css('.property-info__category ::text').get('').strip()

            land_area = response.xpath("//*[contains(text(),'Land Size')]/strong[1]/text()").get('').strip()
            if land_area:
