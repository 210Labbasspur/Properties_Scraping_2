...
class HPG_Sold(scrapy.Spider):
    name = "HPG_Sold"

    url = "https://hpgpropertygroup.com.au/sold"
    prefix = "https://hpgpropertygroup.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(HPG_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyCard>a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = each_url if self.prefix in each_url else f'{self.prefix}{each_url}'
        ...
        next_page = response.xpath("//*[text()='>']/parent::a[1]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2521)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'HPG Estate Agents'
            ...
            # item['Field6'] = response.xpath("//*[@alt='Bedroom']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed-alt')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'fa-fw fa-bath')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car-side')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('.pp_info_header_label_blck ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace('UNDER CONTRACT','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@style,'url(/resize/')]/@style").getall()
            images = []
            for img in prop_images[1:25]:        ## Scrape maximum 25 photos
                url = re.search(r'url\((.*?)\)', img).group(1)
                img = url.replace('background-image:url(','').replace(");","")
                images.append(f'{self.prefix}{img}' if img else "")
            ...
            if response.css(".pp_info_agent_blck"):
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fal fa-fw fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image_style = agent1.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    agent_image = re.search(r'url\((.*?)\)', agent_image_style).group(1)
                    images_string = f'{self.prefix}{agent_image}' if agent_image else ""
            ...
            if len(response.css(".pp_info_agent_blck").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fal fa-fw fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image_style = agent2.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    agent_image = re.search(r'url\((.*?)\)', agent_image_style).group(1)
                    images_string = f'{self.prefix}{agent_image}' if agent_image else ""
            ...
            if len(response.css(".pp_info_agent_blck").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fal fa-fw fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image_style = agent3.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    agent_image = re.search(r'url\((.*?)\)', agent_image_style).group(1)
                    images_string = f'{self.prefix}{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(text(),'Land')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            # floor_area = response.xpath('//*[contains(text(),"Floor area")]/following-sibling::*[1]/text()').get('').strip()
            # if floor_area:
            #     if 'Acres' in floor_area:
            #         pass
            #     else:
            #         item['Field37'] = floor_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            #
            # feature_count = 58
