...
class JanelleStevens_Leased(scrapy.Spider):
    name = "JanelleStevens_Leased"
    url = 'https://www.janellestevens.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.janellestevens.com.au"
    headers = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9,ur;q=0.8,nl;q=0.7',
    'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'origin': 'https://www.janellestevens.com.au',
    'priority': 'u=1, i',
    'referer': 'https://www.janellestevens.com.au/sell/sold-properties/',
    ...
    def __init__(self, *args, **kwargs):
        super(JanelleStevens_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2529)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Janelle Stevens Property'
            ...
            item['Field9'] = response.xpath("//*[text()='Price']/following-sibling::*[1]/text()").get('').strip().replace('$','').replace(',','').replace(' ','')
            item['Field11'] = property.get('sold_at','')
            ...
            if response.css(".listing-agent"):
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.css(".email ::text").get('').strip().replace('E:','').replace('M:','')
                    item['Field20'] = agent1.css(".mobile ::text").get('').strip().replace('E:','').replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field25'] = agent2.css(".email ::text").get('').strip().replace('E:','').replace('M:','')
                    item['Field26'] = agent2.css(".mobile ::text").get('').strip().replace('E:','').replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field30'] = agent3.css(".email ::text").get('').strip().replace('E:','').replace('M:','')
                    item['Field31'] = agent3.css(".mobile ::text").get('').strip().replace('E:','').replace('M:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field40'] = property.get('video_url','')
            item['Field41'] = response.css('.floorplan a ::attr(href)').get('').strip()
            item['Field42'] = response.css('.virtual-tour-items a ::attr(href)').get('').strip()
