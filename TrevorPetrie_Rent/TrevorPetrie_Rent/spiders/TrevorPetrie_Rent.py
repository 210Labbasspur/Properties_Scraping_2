...
class TrevorPetrie_Rent(scrapy.Spider):
    name = "TrevorPetrie_Rent"

    url = "https://www.trevorpetrie.com.au/rent/properties-for-lease/"
    prefix = "https://www.trevorpetrie.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(TrevorPetrie_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".bg-black-5 a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2769)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Trevor Petrie Real Estate'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'las la-bed')]/parent::*[1]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'las la-bath')]/parent::*[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'las la-car')]/parent::*[1]/text()").getall())
            ...
            prop_images = response.xpath("//*[contains(@class,'item-image')]//source/@srcset").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                match = re.search(r'(https://[^\s]+) 1920w', img)
                if match:
                    img = match.group(1)
                else:
                    img = ''
            ...
            if response.css(".listing-agent"):
                ...
                agent1_name = agent1.css('strong p ::text').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".listing-agent").getall()) > 1:
                ...
                agent2_name = agent2.css('strong p ::text').get('').strip()
                if agent2:
            ...
            if len(response.css(".listing-agent").getall()) > 2:
                ...
                agent3_name = agent3.css('strong p ::text').get('').strip()
                if agent3:
            ...
            land_area = response.xpath('//*[contains(@class,"property-land-size")]/text()').get('').strip().replace('Land size','').replace('is','').replace('sqm','').replace(':','')
            if land_area:
