...
class LeadingRE_Sold(scrapy.Spider):
    name = "LeadingRE_Sold"

    url = "https://leadingrealestate.com.au/property/"
    prefix = "https://leadingrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LeadingRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2525)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Leading Real Estte'
            ...
            bbc = response.css('.title-meta-wrapper')
            item['Field6'] = bbc.xpath(".//*[contains(@class,' bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = bbc.xpath(".//*[contains(@class,' bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = bbc.xpath(".//*[contains(@class,' car')]/following-sibling::*[1]/text()").get('').strip()

            price = bbc.css('.page-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.epl-single-section-content-inner ::text').getall())  # description
            ...
            # prop_images = response.css("#epl-carousel-3-5 img ::attr(data-lazy)").getall()
            # prop_images = response.css("#epl-carousel-3-5 img").getall()
            prop_images = response.xpath("//*[contains(@src,'1280x853.jpg') or contains(@data-lazy,'1280x853.jpg')]")
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                if img.css('::attr(data-lazy)'):
                    img = img.css('::attr(data-lazy)').get('').strip()
                else:
                    img = img.css('::attr(src)').get('').strip()
            ...
            if response.css(".epl-author-box-container"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.epl-sd-entry-mobile ::text').get('').strip()
            ...
            if len(response.css(".epl-author-box-container").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css('.epl-sd-entry-mobile ::text').get('').strip()
            ...
            if len(response.css(".epl-author-box-container").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css('.epl-sd-entry-mobile ::text').get('').strip()
            ...
            floor_area = response.xpath("//*[contains(@class,'building-size')]/text()").get('').strip()
            if floor_area:
