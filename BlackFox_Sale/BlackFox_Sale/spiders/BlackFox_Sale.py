...
class BlackFox_Sale(scrapy.Spider):
    name = "BlackFox_Sale"

    url = "https://blackfoxrealestate.com.au/status/for-sale/"
    prefix = "https://blackfoxrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(BlackFox_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-thumb>a ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[@aria-label='Next']/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2528)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Black Fox Real Estate'

            property_street = response.css('.page-title h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/following-sibling::span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/following-sibling::span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(text(),'Garage')]/following-sibling::span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.xpath("//*[contains(text(),'Price')]/following-sibling::span[1]//text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-description-wrap .block-content-wrap ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@id,'property-gallery-js')]//img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".lightbox-form-wrap .agent-details"):
                agent1 = response.css(".lightbox-form-wrap .agent-details")[0]
                agent1_name = agent1.css('img ::attr(alt)').get('').strip()
                if agent1_name:
            ...
            if len(response.css(".lightbox-form-wrap .agent-details").getall()) > 1:
                agent2 = response.css(".lightbox-form-wrap .agent-details")[1]
                agent2_name = agent2.css('img ::attr(alt)').get('').strip()
                if agent2:
            ...
            if len(response.css(".lightbox-form-wrap .agent-details").getall()) > 2:
                agent3 = response.css(".lightbox-form-wrap .agent-details")[2]
                agent3_name = agent3.css('img ::attr(alt)').get('').strip()
                if agent3:
            ...
            land_area = response.xpath("//*[contains(text(),'Property Size')]/following-sibling::span[1]/text()").get('').strip()
            if land_area:
