...
class CoopersAgency_Rent(scrapy.Spider):
    name = "CoopersAgency_Rent"

    url = "https://coopers.agency/properties-for-lease"
    prefix = "https://coopers.agency"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CoopersAgency_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyTile-address ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1099)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Coopers Agency'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyMapTile-address ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'bedroom')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field7'] = response.xpath("//*[contains(text(),'bathroom')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field8'] = response.xpath("//*[contains(text(),'parking')]/*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')

            price = response.css('.propertyPage-property-contracttext ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.user-content p ::text').getall())  # description
            ...
            prop_images = response.css(".propertyPage-carousel-wrap img ::attr(src)").getall()
            images = []
            ...
            if response.css(".agentAvatar"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(src)").get('').strip().replace('100x200','1000x2000')
                    images_string = f'https://coopers.agency{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentAvatar").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(src)").get('').strip().replace('100x200','1000x2000')
                    images_string = f'https://coopers.agency{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentAvatar").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(src)").get('').strip().replace('100x200','1000x2000')
                    images_string = f'https://coopers.agency{agent_image}' if agent_image else ""
            ...
            item['Field40'] = response.xpath("//*[contains(@src,'https://www.youtube')]/@src").get('').strip()
            item['Field41'] = response.css('.propertyPage-floorplanPanel zoom-box ::attr(src)').get('').strip()
            # item['Field42'] = 'virtualwalkthrough'
            # item['Field80'] = 'Contract'
            # item['Field81'] = 'Pest and Building inspection'

            land_area = response.xpath("//*[contains(text(),'land area')]/*[1]/text()").get('').strip()
            if land_area:
