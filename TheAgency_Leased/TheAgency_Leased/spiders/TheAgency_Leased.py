...
class TheAgency_Leased(scrapy.Spider):
    name = "TheAgency_Leased"

    url = "https://theagency.com.au/leased-properties"
    ...
    def __init__(self, *args, **kwargs):
        super(TheAgency_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertyTile a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item['Field6'] = response.xpath("//*[@class='icon-bed']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = response.xpath("//*[@class='icon-bath']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = response.xpath("//*[@class='icon-car']/following-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.leftPanel-itemWrap:nth-child(2) div ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.contentRegion .super ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".contentRegion .contentRegion ::text").getall())  # description
            ...
            prop_images = response.xpath("//*[@itemprop='url']/@href").getall()
            images = []
            ...
            item['Field35'] = response.css('.leftPanel-itemWrap:nth-child(1)>h4 ::text').get('').strip()

            # item['Field40'] = 'video'
            item['Field41'] = response.css('#floorplan-slideshow link ::attr(href)').get('').strip()
