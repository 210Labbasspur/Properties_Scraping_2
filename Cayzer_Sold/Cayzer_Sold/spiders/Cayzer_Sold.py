...
class Cayzer_Sold(scrapy.Spider):
    name = "Cayzer_Sold"

    url = "https://www.cayzer.com.au/buy/recently-sold/list/1"
    prefix = "https://www.cayzer.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Cayzer_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-item>a:nth-child(1) ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//a[text()='>']/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2706)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cayzer Real Estate'

            property_street = response.xpath("//*[contains(@class,'col-xs-4 text-center')]/text()").get('').strip()  # property_street
            property_suburb = response.xpath("//*[contains(@class,'col-xs-4 text-center')]/preceding-sibling::*[1]/text()").get('').strip()  # property_street
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//img[@alt='Bedroom']/following-sibling::text()[1]").get('').strip()
            item['Field7'] = response.xpath("//img[@alt='Bathroom']/following-sibling::text()[1]").get('').strip()
            item['Field8'] = response.xpath("//img[@alt='Car']/following-sibling::text()[1]").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#map-reposition h2 ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'item')]/@style").getall()
            images = []
            ...
            if response.css(".agent-list"):
                ...
                agent1_name = agent1.css('.agent-name b ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[text()='M']/following-sibling::text()[1]").get('').strip()
                    item['Field21'] = agent1.xpath(".//*[text()='P']/following-sibling::text()[1]").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-list").getall()) > 1:
                ...
                agent2_name = agent2.css('.agent-name b ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/text()").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//*[text()='M']/following-sibling::text()[1]").get('').strip()
                    item['Field26A'] = agent2.xpath(".//*[text()='P']/following-sibling::text()[1]").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-list").getall()) > 2:
                ...
                agent3_name = agent3.css('.agent-name b ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/text()").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//*[text()='M']/following-sibling::text()[1]").get('').strip()
                    item['Field31A'] = agent3.xpath(".//*[text()='P']/following-sibling::text()[1]").get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
