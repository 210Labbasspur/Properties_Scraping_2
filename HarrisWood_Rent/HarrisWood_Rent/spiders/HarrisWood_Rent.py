...
class HarrisWood_Rent(scrapy.Spider):
    name = "HarrisWood_Rent"

    url = "https://www.harriswood.com.au/lease/"
    prefix = "https://www.harriswood.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(HarrisWood_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property__link ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.harriswood.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://www.harriswood.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2675)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Harris Wood'

            property_street = response.css('.property-header__left h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.single-property__slider .single-property__title ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street}{property_suburb}"
            item['Field5'] = property_address  # full_adddress

            bbc = []
            for element in response.css('.single-property__meta li'):
                bbc.append(''.join(e.strip() for e in element.css('::text').getall() if e.strip()))
            for e in bbc:
                if 'bed' in e.lower():
                    item['Field6'] = e.strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
                if 'bath' in e.lower():
                    item['Field7'] = e.strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
                if 'car' in e.lower():
                    item['Field8'] = e.strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('.single-property__slider .single-property__meta span ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','').replace('forContactUs','')
            if price:
                item['Field9'] = price
            ...
            '''          AGENTS          '''
            if response.css(".single-property__agent"):
                agent1 = response.css(".single-property__agent")[0]
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('mailto:','').replace('tel:','')
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.xpath(".//*[contains(@data-src,'https')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".single-property__agent").getall()) > 1:
                agent2 = response.css(".single-property__agent")[1]
                agent2_name = agent2.css('h4 span ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('mailto:','').replace('tel:','')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.xpath(".//*[contains(@data-src,'https')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".single-property__agent").getall()) > 2:
                agent3 = response.css(".single-property__agent")[2]
                agent3_name = agent3.css('h4 span ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'@')]/@href").get('').strip().replace('mailto:','').replace('tel:','')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:')]/@href").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.xpath(".//*[contains(@data-src,'https')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            # item['Field40'] = 'video'
            item['Field41'] = response.xpath('//*[contains(text(),"floorplan")]/following-sibling::a[1]/@href').get('').strip()
            # item['Field42'] = 'virtualwalkthrough'
            # item['Field80'] = 'Contract'
            item['Field81'] = response.xpath('//*[contains(text(),"Inspection")]/following-sibling::div[1]//span/text()').get('').strip()
