...
class FlynnandCo_Rent(scrapy.Spider):
    name = "FlynnandCo_Rent"
    url = "https://flynnandco.com.au/listings?saleOrRental=Rental&rental_period=week&status=available_under_contract&viewType=gallery&sortby=dateListed-desc"
    prefix = "https://flynnandco.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(FlynnandCo_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://flynnandco.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://flynnandco.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2647)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Flynn and Co - A Smart Real Estate Agency'
            ...
            if len(response.xpath("//*[contains(@data-testid,'agent-details')]/div").getall()) > 4:
                agent3 = response.xpath("//*[contains(@data-testid,'agent-details')]/div")[-1]
