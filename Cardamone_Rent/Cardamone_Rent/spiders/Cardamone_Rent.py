...
class Cardamone_Rent(scrapy.Spider):
    name = "Cardamone_Rent"

    url = "https://www.cardamonerealestate.com.au/search/?show_in_rental=true&sold=0&tags_ex=storage"
    prefix = "https://www.cardamonerealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Cardamone_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2557)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cardamone Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'attr-bed')]/span/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'attr-bath')]/span/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'attr-car')]/span/text()").get('').strip()
            ...
            prop_images = response.xpath("//*[contains(@data-fancybox,'image-gallery')]/@href").getall()
            images = []
            ...
            if len(response.css(".agent-container").getall()) > 2:
            ...
            land_area = response.xpath("//*[contains(@class,'attr-land')]/span/text()").get('').strip().replace('Land','').replace('is','').replace('m','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(@class,'attr-building')]/span/text()").get('').strip().replace('Land','').replace('is','').replace('m','')
            if floor_area:
            ...
            feature_count = 58
            for feature in response.css(".features-wrapper li"):   ##  first 4 features are ID, bed, bath etc.
