...
class ChandlerandCo_Rent(scrapy.Spider):
    name = "ChandlerandCo_Rent"

    url = "https://chandlerandco.com.au/search-results/?type=Lease&suburb=all&submit=Search&propertytype=&min=&max=&bedroom=&bathroom=&carspace="
    prefix = "https://chandlerandco.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ChandlerandCo_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".figure-anchr ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2690)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Chandler & Co'

            property_street = response.css('.sz-property-heading h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@alt,'icon-1.png')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@alt,'icon-2.png')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@alt,'Sheltered Car Spaces')]/parent::*[1]/text()").get('').strip()

            price = response.css('.bed-detail:nth-child(4) ul li ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.buy-right .buy-com-in ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@data-lightbox,'bannerSlider')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("#enquire div.item"):
                agent1 = response.css("#enquire div.item")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('p ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("#enquire div.item").getall()) > 1:
                agent2 = response.css("#enquire div.item")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css('p ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("#enquire div.item").getall()) > 2:
                agent3 = response.css("#enquire div.item")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css('p ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath("//*[contains(text(),'Land Size: ')]/text()").get('').strip().replace('Land Size: ','').replace('m2','')
            if land_area:
