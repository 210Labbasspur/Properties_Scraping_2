...
class CaseyEstateAgents_Sold(scrapy.Spider):
    name = "CaseyEstateAgents_Sold"

    url = "https://www.caseyestateagents.melbourne/pages/real-estate/results?widgetKey=283015&context=listings&returnAs=markup&listing_sale_method=sale&display_sale_method=Sold&status=sold&pg=1"
    prefix = "https://www.caseyestateagents.melbourne"
    headers = {
    ...
    count = 1
    db = 'CaseyEstateAgents'
    collection = 'CaseyEstateAgents_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(CaseyEstateAgents_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.caseyestateagents.melbourne{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2609)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Casey Estate Agents'
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".description truncated ::text").getall())  # description
            ...
            item['Field35'] = response.css('.property-item-heading ::text').get('').strip()
