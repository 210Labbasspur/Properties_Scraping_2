...
class HomeByHolly_Sale(scrapy.Spider):
    name = "HomeByHolly_Sale"

    url = "https://homebyholly.com.au/for-sale/"
    prefix = "https://homebyholly.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(HomeByHolly_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".col-lg-6 ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2425)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Home By Holly'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'h1 m-0')]/span[1]//text()").getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'meta-bedrooms')]/strong/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'meta-bathrooms')]/strong/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'meta-parking')]/strong/text()").get('').strip()

            price = response.css('.property-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.grey-dark ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'photoswipe-link')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".col-lg-7 .loop-item-agent-preview"):
                agent1 = response.css(".col-lg-7 .loop-item-agent-preview")[0]
                agent1_name = agent1.css('a strong ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//a[contains(@href,'tel:')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent1.xpath(".//a[contains(@href,'tel:')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".avatar ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".col-lg-7 .loop-item-agent-preview").getall()) > 1:
                agent2 = response.css(".col-lg-7 .loop-item-agent-preview")[1]
                agent2_name = agent2.css('a strong ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.xpath(".//a[contains(@href,'tel:')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent2.xpath(".//a[contains(@href,'tel:')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".avatar ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".col-lg-7 .loop-item-agent-preview").getall()) > 2:
                agent3 = response.css(".col-lg-7 .loop-item-agent-preview")[2]
                agent3_name = agent3.css('a strong ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.xpath(".//a[contains(@href,'tel:')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent3.xpath(".//a[contains(@href,'tel:')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[-1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".avatar ::attr(data-lazy-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field35'] = response.xpath("//*[contains(@class,'meta-type')]/strong/text()").get('').strip()
