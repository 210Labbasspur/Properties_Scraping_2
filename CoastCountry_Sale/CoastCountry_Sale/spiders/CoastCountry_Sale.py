...
class CoastCountry_Sale(scrapy.Spider):
    name = "CoastCountry_Sale"

    url = "https://www.coastandcountryfn.com.au/buy/properties-for-sale/"
    prefix = "https://www.coastandcountryfn.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CoastCountry_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2834)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Coast and Country First National Real Estate'
            ...
            prop_images = response.xpath("//*[contains(@class,'slider-image')]//img/@src").getall()
            images = []
            ...
            land_area = land.replace('Land','').replace('is','').replace('sqm','')
            if land_area:
