...
class FrankDowling_Sale(scrapy.Spider):
    name = "FrankDowling_Sale"
    url = ('https://www.frankdowling.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bcount%5D%5Bvalue%5D=20&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Bmeta_key%5D%5Bvalue%5D=dateListed&query%5Bsold%5D%5Bvalue%5D=0&query%5BsaleOrRental%5D%5Bvalue%5D=sale&query%5BsaleOrRental%5D%5Btype%5D=equal&query'
           '%5Bpaged%5D%5Bvalue%5D={}')
    prefix = "https://www.frankdowling.com.au"
    headers = {
    ...
    count = 1
    db = 'FrankDowling'
    collection = 'FrankDowling_Sale'
    bucket_prefix = f'Images6'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(FrankDowling_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2630)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Frank Dowling Estate Agents'
