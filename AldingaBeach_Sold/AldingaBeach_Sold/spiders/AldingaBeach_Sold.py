...
class AldingaBeach_Sold(scrapy.Spider):
    name = "AldingaBeach_Sold"

    url = "https://www.aldingabeachres.com.au/recently-sold"
    prefix = "https://www.aldingabeachres.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AldingaBeach_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".figure ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.aldingabeachres.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3373)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Aldinga Beach Real Estate Service'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'flaticon-person1 big')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'flaticon-shower5')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'flaticon-car95')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.sidebar h1 ::text').getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".card-head"):
                agent1 = response.css(".card-head")[0]
            ...
            if len(response.css(".card-head").getall()) > 1:
                agent2 = response.css(".card-head")[1]
            ...
            if len(response.css(".card-head").getall()) > 2:
                agent3 = response.css(".card-head")[2]
            ...
            land_area = response.xpath("//*[contains(@class,'flaticon-display6')]/parent::*[1]/text()").get('').strip().lower().replace('square metres','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            if land_area:
