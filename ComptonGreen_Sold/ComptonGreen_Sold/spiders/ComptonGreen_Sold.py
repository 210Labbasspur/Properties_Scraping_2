...
class ComptonGreen_Sold(scrapy.Spider):
    name = "ComptonGreen_Sold"

    url = "https://www.comptongreen.com.au/recently-sold"
    prefix = "https://www.comptongreen.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ComptonGreen_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".img-wrap>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2579)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Compton Green'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'flaticon-person1 big')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field7'] = response.xpath("//*[contains(@class,'flaticon-shower5')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field8'] = response.xpath("//*[contains(@class,'flaticon-car95')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            ...
            prop_images = response.xpath("//*[contains(@class,'swipebox')]/@href").getall()
            images = []
            ...
            item['Field33'] = response.xpath("//*[@name='property_id']/@value").get('').strip()
            ...
            item['Field81'] = response.xpath("//*[contains(@class,'fa fa-calendar')]/parent::*[1]/text()").get('').strip()

            land_area = response.xpath("//*[contains(@class,'flaticon-display6')]/parent::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Square metres','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
