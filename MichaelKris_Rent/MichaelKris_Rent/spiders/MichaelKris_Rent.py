...
class MichaelKris_Rent(scrapy.Spider):
    name = "MichaelKris_Rent"

    url = "https://www.michaelkris.com.au/renting/properties-for-lease/"
    prefix = "https://www.michaelkris.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MichaelKris_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2451)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Michael Kris Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/following-sibling::*[1]/text()").get('').strip()

            pre_price = response.xpath("//*[contains(text(),'Price')]/following-sibling::*[1]/text()").get('').strip()
            price = pre_price.replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-description-wrapper .color-primary ::text').getall())  # description
            ...
            item['Field33'] = response.xpath("//label[contains(text(),'Property ID')]/following-sibling::*[1]/text()").get('').strip()
            item['Field35'] = response.xpath("//label[contains(text(),'Type')]/following-sibling::*[1]/text()").get('').strip()
            ...
            floor_area = response.xpath('//*[contains(text(),"Building Size")]/following-sibling::*[1]/text()').get('').strip()
            if floor_area:
