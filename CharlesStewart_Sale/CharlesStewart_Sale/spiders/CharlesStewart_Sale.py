...
class CharlesStewart_Sale(scrapy.Spider):
    name = "CharlesStewart_Sale"

    url = "https://charlesstewart.com.au/residential/residential-properties"
    prefix = "https://charlesstewart.com.au"
    headers = {
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'accept-language': 'en-US,en;q=0.9',
            'priority': 'u=0, i',
            'referer': 'https://charlesstewart.com.au/residential/sold-properties',
            ...
            # 'cookie': '_gid=GA1.3.1196198411.1742571641; _gcl_au=1.1.1230027576.1742572369; PHPSESSID=bsolpvmm8buart5h5n16nr6rj5; _ga_LPL9926RZT=GS1.1.1742595634.2.1.1742595671.0.0.0; _ga=GA1.1.1195217167.1742571641; _ga_BCYFV9EXYR=GS1.3.1742595638.2.1.1742595672.0.0.0; _ga_ESQF96NGKN=GS1.3.1742595638.2.1.1742595673.0.0.0',
        }


    count = 1
    db = 'CharlesStewart'
    collection = 'CharlesStewart_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(CharlesStewart_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".hvr-shadow ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2519)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Charles Stewart'

            property_street = response.css('.propertySubHeading ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyHeading ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon bed')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon bath')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon car')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[contains(@class,'col-md-6 col-sm-6 col-xs-12 pull-left')]//h3/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#propertyPulloutWrapper .text-left ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#propertyDescriptionWrapper p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@rel,'gallery')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("#propertyAgentSlider .item"):
                agent1 = response.css("#propertyAgentSlider .item")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".imgWrapper ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
            ...
            if len(response.css("#propertyAgentSlider .item").getall()) > 1:
                agent2 = response.css("#propertyAgentSlider .item")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".imgWrapper ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            if len(response.css("#propertyAgentSlider .item").getall()) > 2:
                agent3 = response.css("#propertyAgentSlider .item")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".imgWrapper ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'https:{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//small[contains(text(),'m')]/parent::h3[1]/text()").get('').strip()
            if land_area:
