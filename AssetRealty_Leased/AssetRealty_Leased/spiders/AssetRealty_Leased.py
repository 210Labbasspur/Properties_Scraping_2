...
class AssetRealty_Leased(scrapy.Spider):
    name = "AssetRealty_Leased"

    url = "https://www.assetrealty.com.au/lease/leased-properties/"
    prefix = "https://www.assetrealty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(AssetRealty_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1475)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Asset Realty'
            ...
            bbc = response.css('#quickinfo')
            # item['Field6'] = response.xpath("//*[@alt='Bedroom']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field6'] = bbc.xpath(".//*[contains(@class,'la-bed')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = bbc.xpath(".//*[contains(@class,'la-bath')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = bbc.xpath(".//*[contains(@class,'la-car')]/parent::*[1]/text()[2]").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            prop_images = response.xpath("//*[contains(@class,'img-default-used-size-hd')]/@src").getall()
            images = []
            ...
            # item['Field40'] = 'video'
            item['Field41'] = response.css('.floorplan a ::attr(href)').get('').strip()
            # item['Field42'] = 'virtualwalkthrough'
            # item['Field80'] = 'Contract'
            item['Field81'] = response.css('.auction-item__info .date-time ::text').get('').strip()
