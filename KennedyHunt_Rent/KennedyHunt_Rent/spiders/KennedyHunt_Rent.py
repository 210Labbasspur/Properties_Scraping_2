...
class KennedyHunt_Rent(scrapy.Spider):
    name = "KennedyHunt_Rent"

    url = "https://www.kennedyandhunt.com.au/renting/properties-for-lease/"
    prefix = "https://www.kennedyandhunt.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(KennedyHunt_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2539)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Kennedy & Hunt Real Estate'
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-description-wrapper ::text").getall())  # description
            ...
            floor_area = response.xpath("//*[contains(text(),'Building Size')]/following-sibling::*[1]/text()").get('').strip().replace('Land','').replace('is','').replace('m²','')
            if floor_area:
