...
class SEJ_Sale(scrapy.Spider):
    name = "SEJ_Sale"

    url = "https://www.sej.com.au/search/?show_in_sale=true&sold=1&order_by=sold_date&order=desc"
    prefix = "https://www.sej.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(SEJ_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".card-image a ::attr(href)").getall()
        links = []
        ...
        next_page = response.xpath("//*[contains(@class,'fa-chevron-right')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2818)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'SEJ Livestock & Real Estate'

            property_street = response.css('.properties-details-title h4 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bed')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bath')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'parking')]/parent::*[1]/text()").get('').strip()

            price = response.css('.properties-details-title .text-orange ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.description-text ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'lazy gallery-image')]/@href").getall()
            images = []
            ...
            if response.css(".agents-profile"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agents-profile").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agents-profile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(@class,'land-size')]/parent::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('m','').replace(' ','')   # land_area
            ...
            #
            feature_count = 58
            for feature in response.css(".features-content"):   ##  first 4 features are ID, bed, bath etc.
