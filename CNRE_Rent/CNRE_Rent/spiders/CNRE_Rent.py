...
class CNRE_Rent(scrapy.Spider):
    name = "CNRE_Rent"

    url = "https://cnre.com.au/rent/?type=Lease"
    ...
    def __init__(self, *args, **kwargs):
        super(CNRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".col-md-5 .view_property ::attr(href)").getall()
        links = []
        for each_url in all_links:
            id = re.search(r'pid=([^&]+)', each_url).group(1)
            # each_url = f'https://cnre.com.au/property/?pid={id}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            property_suburb = ' '.join(e.strip() for e in response.css('#details-middle h1 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'ab-bed ab_bbc')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'ab-bath ab_bbc')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'ab-car ab_bbc')]/text()").get('').strip()

            price = ''.join(e.strip() for e in response.css('.price p ::text').getall())
            ...
            land = response.xpath("//*[contains(text(),'Home Size')]/following-sibling::*[1]/text()").get('').strip()
