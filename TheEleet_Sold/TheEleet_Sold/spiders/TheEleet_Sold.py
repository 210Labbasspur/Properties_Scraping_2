...
class TheEleet_Sold(scrapy.Spider):
    name = "TheEleet_Sold"

    url = "https://theeleetwyndham.com.au/property-status/sold/"
    prefix = "https://theeleetwyndham.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(TheEleet_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".top-info .property-title>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2625)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'The Eleet'

            property_street = response.css('h1.property-title ::text').get('').strip()  # property_street
            ...
            bbc = response.css('.left-infor')
            item['Field6'] = bbc.xpath(".//*[contains(@class,'flaticon-bed')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','')
            item['Field7'] = bbc.xpath(".//*[contains(@class,'flaticon-bath')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','')
            item['Field8'] = bbc.xpath(".//*[contains(@class,'flaticon-car')]/following-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','')

            price = response.css('.price-text ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.description-inner-wrapper ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'p-popup-image')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-content-wrapper"):
                agent1 = response.css(".agent-content-wrapper")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.xpath(".//*[contains(@data-src,'http')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".agent-content-wrapper").getall()) > 1:
                agent2 = response.css(".agent-content-wrapper")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.xpath(".//*[contains(@data-src,'http')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-content-wrapper").getall()) > 2:
                agent3 = response.css(".agent-content-wrapper")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.xpath(".//*[contains(@data-src,'http')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(text(),'Lot area:')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
