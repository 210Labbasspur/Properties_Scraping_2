...
class SmartRE_Sold(scrapy.Spider):
    name = "SmartRE_Sold"

    url = "https://www.smartrealestate.co.nz/recently-sold"
    prefix = "https://www.smartrealestate.co.nz"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(SmartRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.smartrealestate.co.nz{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2717)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Smart Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.inner_desc h3 ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@src,'bed-white.png')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@src,'bath-white.png')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@src,'car-white.png')]/parent::*[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Enquiries Over','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            #
            feature_count = 58
            for feature in response.css("#tab-features .active"):   ##  first 4 features are ID, bed, bath etc.
