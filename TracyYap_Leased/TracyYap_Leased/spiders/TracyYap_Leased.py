...
class TracyYap_Leased(scrapy.Spider):
    name = "TracyYap_Leased"

    url = "https://tracyyap.com.au/recent-leased-properties"
    ...
    def __init__(self, *args, **kwargs):
        super(TracyYap_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".searchResults a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1900)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Tracy Yap Realty'

            bbc = response.css('.highlight')
            property_street = bbc.css('.streetaddress ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in bbc.css('.suburb ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = bbc.xpath(".//*[@class='icon-bed']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = bbc.xpath(".//*[@class='icon-bath']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = bbc.xpath(".//*[@class='icon-car']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.column .text-left-med small.block ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            img_string = response.xpath("//*[contains(text(),'media only screen')]/text()").get('').strip()
            prop_images = re.findall(r'url\((https.*?1024x680.*?)\)', img_string)
            images = []
            ...
            if response.css(".agentRow"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".bg-cover ::attr(style)").get('').strip().replace('border-radius: 50%; background-position: top left; background-image: url(','').replace(');','').replace('100x300','1024x680')
                    images_string = f'https://assets2-tracyyap.webfrontcms.net{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentRow").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".bg-cover ::attr(style)").get('').strip().replace('border-radius: 50%; background-position: top left; background-image: url(','').replace(');','').replace('100x300','1024x680')
                    images_string = f'https://assets2-tracyyap.webfrontcms.net{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentRow").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".bg-cover ::attr(style)").get('').strip().replace('border-radius: 50%; background-position: top left; background-image: url(','').replace(');','').replace('100x300','1024x680')
                    images_string = f'https://assets2-tracyyap.webfrontcms.net{agent_image}' if agent_image else ""
            ...
            # item['Field40'] = 'video'
            item['Field41'] = response.css('.bg-contain ::attr(style)').get('').strip().replace('z-index: -1; background-image: url(','').replace(')','')
