##################333           MelbourneSothebysRealty_Sale
...
class MelbourneSothebysRealty_Sale(scrapy.Spider):
    name = "MelbourneSothebysRealty_Sale"

    url = "https://melbournesothebysrealty.com/properties-for-sale?max=999999999&orderby=datedesc&searchtype=1&page=1"
    prefix = "https://melbournesothebysrealty.com"
    headers = {
    ...
    count = 1
    db = 'MelbourneSothebysRealty'
    collection = 'MelbourneSothebysRealty_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(MelbourneSothebysRealty_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2661)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Melbourne Sothebys International Realty'

            property_street = response.css('.propertyMapTile-address-streetAddress ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.propertyMapTile-address-suburb ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'bedroom')]/span//text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'bathroom')]/span//text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'parking')]/span//text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h2.propertyPage-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".propertyPage-property-description .user-content p ::text").getall())  # description
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    images_string = f'https://melbournesothebysrealty.com{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(text(),'land area')]/span//text()").get('').strip()
            if land_area:
