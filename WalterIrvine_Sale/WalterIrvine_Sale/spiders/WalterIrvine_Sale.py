...
class WalterIrvine_Sale(scrapy.Spider):
    name = "WalterIrvine_Sale"

    url = "https://walterirvine.com.au/properties/"
    prefix = "https://walterirvine.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(WalterIrvine_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".propertybox ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://walterirvine.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(text(),'More results »')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2480)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Walter & Irvine Real Estate'

            property_street = response.css('.contentinner h1 .price ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.contentinner h1 .subtitle ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}".replace('Sold','').replace('in','').replace('\r','').replace('\n','').replace('\t','').replace('  ','')
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/parent::*[1]/text()").get('').strip().replace('bedroom','').replace('s','')
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bathroom')]/parent::*[1]/text()").get('').strip().replace('bathroom','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/parent::*[1]/text()").get('').strip().replace('car space','').replace('s','')

            price = response.css('.xxxxxxxxxxxxxxx ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.propertyinner ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@data-lightbox,'property')]/@href").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('background-image:url(','').replace(");","")
                images.append(f'https://walterirvine.com.au{img}' if img else "")
            ...
            '''          AGENTS         - No Agents on sold pages of the website '''
