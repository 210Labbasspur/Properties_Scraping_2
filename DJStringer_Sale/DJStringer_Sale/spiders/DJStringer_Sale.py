...
class DJStringer_Sale(scrapy.Spider):
    name = "DJStringer_Sale"

    url = "https://www.djstringer.com.au/buy/residential-listings/"
    prefix = "https://www.djstringer.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DJStringer_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2039)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'DJStringer'

            property_street = response.css('.property-address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-suburb ::text').getall() if e.strip())  # property_suburb
