...
class Danckert_Sale(scrapy.Spider):
    name = "Danckert_Sale"

    url = "https://www.danckert.com.au/sale.html"
    prefix = "https://www.danckert.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Danckert_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.danckert.com.au/{each_url}' if '.html?id=' in each_url else None
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2540)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Danckert Real Estate'

            property_street = response.css('.listing-wrapper h1 ::text').get('').strip()  # property_street
            ...
            item['Field8'] = response.xpath("//*[@class='car']//text()").get('').strip()

            price = response.css('#price .card-body ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.blurb tbody tr ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'slide h-100')]/img/@src").getall()
            images = []
            ...
            text = response.css('.contact-info p::text').get('').strip()
            name, number = text.split('\xa0')
            agent1_name = name.strip()
            first_name1, last_name1 = agent1_name.split(maxsplit=1)
            item['Field15'] = first_name1.strip()  # agent_first_name_1
            item['Field16'] = last_name1.strip()  # agent_surname_name_1
            item['Field17'] = name.strip()  # agent_full_name_1
            item['Field20'] = number.strip()
            '''          AGENTS          '''
