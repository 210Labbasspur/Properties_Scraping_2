...
class Rendina_Sold(scrapy.Spider):
    name = "Rendina_Sold"

    url = "https://www.rendina.com.au/pages/real-estate/results?widgetKey=61441&context=listings&returnAs=markup&listing_sale_method=sale&listing_bathrooms=1&display_sale_method=SOLD&status=sold&category=Residential&listing_bedrooms=1&carstotal=1&pg=1"
    prefix = "https://www.rendina.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Rendina_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-data-wrapper ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.rendina.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(text(),'VIEW MORE')]/@hx-get").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2711)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Rendina Real Estate'

            property_street = response.css('.listing-address .listing-heading ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//span[contains(text(),'Beds')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//span[contains(text(),'Baths')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//span[contains(text(),'Cars')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[contains(@class,'display-price')]/span[4]").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@class,'lightbox-image')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-body"):
                agent1 = response.css(".agent-body")[0]
                agent1_name = agent1.css('.line_1 ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field20'] = agent1.css('.line_2 ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-body").getall()) > 1:
                agent2 = response.css(".agent-body")[1]
                agent2_name = agent2.css('.line_1 ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field26'] = agent2.css('.line_2 ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-body").getall()) > 2:
                agent3 = response.css(".agent-body")[2]
                agent3_name = agent3.css('.line_1 ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field31'] = agent3.css('.line_2 ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            land_area = response.xpath('//span[contains(text(),"Land ")]/text()').get('').strip().replace('Land','').replace('is','').replace('m²','').replace(',','')
            if land_area:
