...
class Sweeneye_Sold(scrapy.Spider):
    name = "Sweeneye_Sold"
    url = 'https://www.sweeneyea.com.au/wp-admin/admin-ajax.php'
    prefix = "https://www.sweeneyea.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Sweeneye_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2376)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Sweeneye'
            ...
            item['Field8'] = response.xpath("//*[contains(@class,'carspace')]//text()").get('').strip()
            ...
            '''          AGENTS          '''
            if response.css(".team-item"):
                agent1 = response.css(".team-item")[0]
                ...
                if agent1_name:
                    ...
                    item['Field19'] = agent1.css('.agent-email a ::text').get('').strip()
                    item['Field20'] = agent1.css(".agent-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.css(".agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".team-item").getall()) > 1:
                agent2 = response.css(".team-item")[1]
                agent2_name = agent2.css('.agent-name a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.css('.agent-email a ::text').get('').strip()
                    item['Field26'] = agent2.css(".agent-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.css(".agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".team-item").getall()) > 2:
                agent3 = response.css(".team-item")[2]
                agent3_name = agent3.css('.agent-name a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.css('.agent-email a ::text').get('').strip()
                    item['Field31'] = agent3.css(".agent-phone a ::attr(href)").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.css(".agent-mobile a ::attr(href)").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
