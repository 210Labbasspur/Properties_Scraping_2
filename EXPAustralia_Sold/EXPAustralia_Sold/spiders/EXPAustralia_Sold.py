...
class EXPAustralia_Sold(scrapy.Spider):
    name = "EXPAustralia_Sold"

    url = "https://www.expaustralia.com.au/sell/sold-properties/"
    prefix = "https://www.expaustralia.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(EXPAustralia_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2057)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'EXP Realty Australia'
            ...
            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('Price :','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#property-description .section-title ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#property-description .copy ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.css(".property-slider-main img ::attr(src)").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".contact-agent-box .agent"):
                agent1 = response.css(".contact-agent-box .agent")[0]
            ...
            if len(response.css(".contact-agent-box .agent").getall()) > 1:
                agent2 = response.css(".contact-agent-box .agent")[1]
            ...
            if len(response.css(".contact-agent-box .agent").getall()) > 2:
                agent3 = response.css(".contact-agent-box .agent")[2]
