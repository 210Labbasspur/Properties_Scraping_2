...
class MckeanMcgregor_Sale(scrapy.Spider):
    name = "MckeanMcgregor_Sale"

    url = "https://www.mckeanmcgregor.com.au/pages/real-estate/results?widgetKey=192730&context=listings&returnAs=markup&listing_category=residential&listing_sale_method=sale&display_sale_method=BUY&office_id=139&pg=1"
    prefix = "https://www.mckeanmcgregor.com.au"
    headers = {
    ...
    count = 1
    db = 'MckeanMcgregor'
    collection = 'MckeanMcgregor_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(MckeanMcgregor_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.mckeanmcgregor.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(@hx-get,'/pages')]/@hx-get").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2643)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Mckean Mcgregor'

            property_street = response.css('.xxxxxxxxxxxx ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//div[contains(text(),'Bed')]/text()").get('').strip().replace('Bed','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field7'] = response.xpath("//div[contains(text(),'Bath')]/text()").get('').strip().replace('Bath','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field8'] = response.xpath("//div[contains(text(),'Car')]/text()").get('').strip().replace('Car','').replace('s','').replace('\n','').replace('\xa0','')
            ...
            #
            feature_count = 58
            for feature in response.css(".property-item-content"):   ##  first 4 features are ID, bed, bath etc.
