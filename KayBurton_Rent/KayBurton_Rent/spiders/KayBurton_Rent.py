...
class KayBurton_Rent(scrapy.Spider):
    name = "KayBurton_Rent"

    url = "https://www.kayburton.com.au/rent/unfurnished-properties/"
    prefix = "https://www.kayburton.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(KayBurton_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        yield scrapy.Request('https://www.kayburton.com.au/rent/furnished-properties/', headers=self.headers, callback=self.parse)

    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".address a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2523)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Kay & Burton'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.sidebar .address ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            bbc = response.css('.sidebar ')
            item['Field6'] = ''.join(e.strip() for e in bbc.css(".bedroom ::text").getall())
            item['Field7'] = ''.join(e.strip() for e in bbc.css(".bathroom ::text").getall())
            item['Field8'] = ''.join(e.strip() for e in bbc.css(".carspace ::text").getall())

            price = bbc.css('.price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@class,'image-gallery-item item')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agents .agent"):
                agent1 = response.css(".agents .agent")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('.name a ::text').getall())
                if agent1_name:
                    ...
                    item['Field19'] = agent1.xpath(".//div[contains(@class,'email')]//a//text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".agents .agent").getall()) > 1:
                agent2 = response.css(".agents .agent")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('.name a ::text').getall())
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.role ::text').get('').strip()
                    item['Field25'] = agent2.xpath(".//div[contains(@class,'email')]//a//text()").get('').strip().replace('mailto:','')
            ...
            if len(response.css(".agents .agent").getall()) > 2:
                agent3 = response.css(".agents .agent")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('.name a ::text').getall())
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.role ::text').get('').strip()
                    item['Field30'] = agent3.xpath(".//div[contains(@class,'email')]//a//text()").get('').strip().replace('mailto:','')
