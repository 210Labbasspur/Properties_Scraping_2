...
class Boffo_Sold(scrapy.Spider):
    name = "Boffo_Sold"

    url = "https://boffo.com.au/sell/sold"
    prefix = "https://boffo.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Boffo_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        next_page = response.xpath("//*[contains(@data-target,'next')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2404)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Boffo'

            property_street = response.css('.hero__title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'icon-bed')]/parent::*[1]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'icon-bath')]/parent::*[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'icon-car')]/parent::*[1]/text()").getall())

            item['Field9'] = response.css('.hero__subtitle ::text').get('').strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.block--rich-text ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'listing__item')]//img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".team-member"):
                agent1 = response.css(".team-member")[0]
                agent1_name = agent1.css('.team-member__name ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.team-member__position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".lazy__image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".team-member").getall()) > 1:
                agent2 = response.css(".team-member")[1]
                agent2_name = agent2.css('.team-member__name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.team-member__position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".lazy__image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".team-member").getall()) > 2:
                agent3 = response.css(".team-member")[2]
                agent3_name = agent3.css('.team-member__name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.team-member__position ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".lazy__image ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
