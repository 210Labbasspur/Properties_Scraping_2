...
class PeakeRE_Sale(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".properties__items a ::attr(href)").getall()
        links = []
        ...
        for property_url in all_links:
            property_url = f'https://peakere.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2514)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Peake Real Estate'

            property_street = response.css('.property-template__image img ::attr(alt)').get('').strip()  # property_street
            ...
            bbc = []
            for element in response.css('.property-template__header .property-template__stats span'):
            ...
            for e in bbc:
                ...
                if 'm2' in e.lower():
                    item['Field36'] = e.strip().lower().replace('m2','')

            price = response.css('.property-template__header .property-template__stats p ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if price:
                item['Field9'] = price
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-template__description ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@class='slick-slide slick-cloned']//img/@src").getall()
            images = []
            images.append(response.css('.property-template__image img ::attr(src)').get('').strip())
            ...
            #
            feature_count = 58
            for feature in response.css(".property-template__key-features li"):   ##  first 4 features are ID, bed, bath etc.
