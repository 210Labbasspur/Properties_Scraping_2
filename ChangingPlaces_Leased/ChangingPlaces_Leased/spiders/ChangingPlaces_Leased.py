...
class ChangingPlaces_Leased(scrapy.Spider):
    name = "ChangingPlaces_Leased"

    url = "https://changingplaces.com.au/rent/recently-let/"
    prefix = "https://changingplaces.com.au"
    headers = {
    ...
    count = 1
    db = 'ChangingPlaces'
    collection = 'ChangingPlaces_Leased'
    ...
    def __init__(self, *args, **kwargs):
        super(ChangingPlaces_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".item-wrap .image>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2654)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Changing Places'

            property_street = response.css('#realty_widget_media .section_title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bedrooms')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'bathrooms')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'carspaces')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('#property_price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('#realty_widget_property_description ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@rel,'prettyPhoto[photo_gallery]')]/@href").getall()
            images = []
            ...
            if response.css(".agent_info"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.agent_mobile span ::text').get('').strip()
                    item['Field21'] = agent1.css('.agent_phone span ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent_info").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.css('.agent_mobile span ::text').get('').strip()
                    item['Field26A'] = agent2.css('.agent_phone span ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent_info").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.css('.agent_mobile span ::text').get('').strip()
                    item['Field31A'] = agent3.css('.agent_phone span ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field35'] = response.css('.property_type span ::text').get('').strip()
