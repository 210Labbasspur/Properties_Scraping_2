...
class MorrisonKleeman_Sold(scrapy.Spider):
    name = "MorrisonKleeman_Sold"

    url = "https://www.morrisonkleeman.com.au/search-list?type=Sold"
    prefix = "https://www.morrisonkleeman.com.au"
    headers = {
    ...
    count = 1
    db = 'MorrisonKleeman'
    collection = 'MorrisonKleeman_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(MorrisonKleeman_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-image a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.morrisonkleeman.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(@class,'fa fa-caret-right')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2705)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'MorrisonKleeman'

            property_street = response.css('.container h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/following-sibling::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/following-sibling::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon-garage')]/following-sibling::span[1]/text()").get('').strip()

            price = response.xpath("//*[contains(@class,'fa fa-money')]/following-sibling::span[1]/text()").get('').strip()
            ...
            prop_images = response.xpath("//*[contains(@class,'magnific-item')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css("div.ld-agent-grid-card"):
                agent1 = response.css("div.ld-agent-grid-card")[0]
                agent1_name = agent1.css('.ld-agent-link ::text').get('').strip()
                if agent1_name:
                    ...
                    item['Field20'] = agent1.css('.ld-agent-grid-card-phone .align-mid ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".ld-agent-grid-card-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
            ...
            if len(response.css("div.ld-agent-grid-card").getall()) > 1:
                agent2 = response.css("div.ld-agent-grid-card")[1]
                agent2_name = agent2.css('.ld-agent-link ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field25'] = agent2.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field26'] = agent2.css('.ld-agent-grid-card-phone .align-mid ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".ld-agent-grid-card-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css("div.ld-agent-grid-card").getall()) > 4:
                agent3 = response.css("div.ld-agent-grid-card")[2]
                agent3_name = agent3.css('.ld-agent-link ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field30'] = agent3.xpath(".//a[contains(@href,'mailto:')]/@href").get('').strip().replace('mailto:', '')
                    item['Field31'] = agent3.css('.ld-agent-grid-card-phone .align-mid ::text').get('').strip()
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".ld-agent-grid-card-image ::attr(style)").get('').strip().replace('background-image:url(','').replace(');','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            item['Field33'] = response.url.split("listing_id=")[-1]
            # item['Field35'] = response.css('.ididididididid ::text').get('').strip()

            land = response.xpath("//*[contains(@class,'fa fa-expand')]/following-sibling::span[1]/text()").get('').strip()
