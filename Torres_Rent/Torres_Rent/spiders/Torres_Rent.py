...
class Torres_Rent(scrapy.Spider):
    name = "Torres_Rent"

    url = "https://www.torresproperty.com.au/lease/"
    prefix = "https://www.torresproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Torres_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2328)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Torres Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'Beds')]/span[contains(@class,'listing-details-value')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'Baths')]/span[contains(@class,'listing-details-value')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'Parking')]/span[contains(@class,'listing-details-value')]/text()").get('').strip()
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".desktop-desc ::text").getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".wpsight-listing-agent"):
                agent1 = response.css(".wpsight-listing-agent")[0]
            ...
            if len(response.css(".wpsight-listing-agent").getall()) > 1:
                agent2 = response.css(".wpsight-listing-agent")[1]
            ...
            if len(response.css(".wpsight-listing-agent").getall()) > 2:
                agent3 = response.css(".wpsight-listing-agent")[2]
            ...
            land = response.xpath("//*[contains(@title,'Land Size')]/span[contains(@class,'listing-details-value')]/text()").get('').strip()
            land_area = land.replace('m2','').replace('Land','').replace('is','').replace('m²','')
            if land_area:
