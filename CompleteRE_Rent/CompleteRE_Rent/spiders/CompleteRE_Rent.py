...
class CompleteRE_Rent(scrapy.Spider):
    name = "CompleteRE_Rent"

    url = "https://www.completerealestate.net.au/rent/"
    prefix = "https://www.completerealestate.net.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CompleteRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".epl-property-blog-rec-default a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(3375)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Complete Real Estate'

            property_street = response.css('#icons .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('#icons .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.css(".bedroom .rec-icon-value ::text").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.css(".bathroom .rec-icon-value ::text").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.css(".garage .rec-icon-value ::text").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.epl-section-inner__panel--description ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'epl-popupslider-item epl-popupslider-item')]//img/@src").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('-760x380','')
            ...
            '''          AGENTS          '''
            if response.css(".epl-section-inner__wrapper .epl-author-box-card__inner"):
                agent1 = response.css(".epl-section-inner__wrapper .epl-author-box-card__inner")[0]
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(src)").get('').strip().replace('-300x310','')
            ...
            if len(response.css(".epl-section-inner__wrapper .epl-author-box-card__inner").getall()) > 1:
                agent2 = response.css(".epl-section-inner__wrapper .epl-author-box-card__inner")[1]
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(src)").get('').strip().replace('-300x310','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".epl-section-inner__wrapper .epl-author-box-card__inner").getall()) > 2:
                agent3 = response.css(".epl-section-inner__wrapper .epl-author-box-card__inner")[2]
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(src)").get('').strip().replace('-300x310','')
                    images_string = f'{agent_image}' if agent_image else ""
