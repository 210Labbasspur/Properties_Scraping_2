...
class MooneeValley_Sale(scrapy.Spider):
    name = 'MooneeValley_Sale'
    url = "https://www.mooneevalley.com.au/propertiesJson?callback=angular.callbacks._0&currentPage={}&perPage=12&sort=d_listing%20desc&listing_cat0=residential&categories=House"
    prefix = 'https://www.mooneevalley.com.au'
    headers = {
        'accept': '*/*',
    ...
    count = 1
    db = 'MooneeValley'
    collection = 'MooneeValley_Sale'
    bucket_prefix = f'D_{collection}'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(MooneeValley_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def Detail_parse(self, mini_data,full_data):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2767)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Moonee ValleyReal Estate'
            ...
            html_string = full_data.get('fields').get('desc_full_html').strip().replace('&nbsp;','')
