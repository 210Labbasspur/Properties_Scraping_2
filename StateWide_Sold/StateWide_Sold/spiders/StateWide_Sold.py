...
class StateWide_Sold(scrapy.Spider):
    name = "StateWide_Sold"

    url = "https://www.statewideproperty.net/our-results/sold-properties"
    prefix = "https://www.statewideproperty.net"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(StateWide_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".renet_listing_item_parent>a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2472)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'State Wide Property'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/parent::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            ...
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('background-image:url("','').replace('");','').replace('background-size:;','').replace(' ','').replace('\n','').replace('\t','').replace('\r','')
            ...
            land_area = response.xpath("//*[contains(text(),'Land Size')]/preceding-sibling::*[1]/text()").get('').strip().replace('sqm approx.','')
            if land_area:
