...
class VicProp_Rent(scrapy.Spider):
    name = "VicProp_Rent"

    url = "https://vicprop.com.au/rent/"
    prefix = "https://vicprop.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(VicProp_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("._12sh105y ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://vicprop.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2752)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'VICPROP'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('._1gvm1kdj ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car Spaces')]/preceding-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[contains(text(),'Price')]/preceding-sibling::*[1]/text()").get('').strip()
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'_1bsb1osq _vwz41tcg _1wybw1gv _syazu67f _p12f1osq _1gvm1swo')]/text()").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}".replace('Read More','')  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'_uwwnglyw')]//img/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]"):
                agent1 = response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]")[0]
                agent1_name = agent1.css('._hy0xf6fq ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.xpath(".//*[contains(@class,'_vwz41tcg _syaz185y _n3td1l7b')]/text()").get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.xpath(".//img[contains(@data-src,'')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://vicprop.com.au{agent_image}' if agent_image else ""
                    # item['Field22'] = images_string
                    # print('Agent1 image is :',images_string)
                    random_number = random.randint(1, 10000000000)
                    item['Field22'] = ",".join(self.uploaded(images_string, random_number))

            if len(response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]").getall()) > 1:
                agent2 = response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]")[1]
                agent2_name = agent2.css('._hy0xf6fq ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.xpath(".//*[contains(@class,'_vwz41tcg _syaz185y _n3td1l7b')]/text()").get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.xpath(".//img[contains(@data-src,'')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://vicprop.com.au{agent_image}' if agent_image else ""
            ...
            if len(response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]").getall()) > 2:
                agent3 = response.xpath("//*[contains(@class,'_1e0c1txw _ca0q19bv')]")[2]
                agent3_name = agent3.css('._hy0xf6fq ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.xpath(".//*[contains(@class,'_vwz41tcg _syaz185y _n3td1l7b')]/text()").get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.xpath(".//img[contains(@data-src,'')]/@data-src").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'https://vicprop.com.au{agent_image}' if agent_image else ""
