...
class RyderRE_Rent(scrapy.Spider):
    name = "RyderRE_Rent"

    url = "https://www.ryderrealestate.com.au/search/?show_in_rental=true&sold=0&order_by=listing_date&order=desc"
    prefix = "https://www.ryderrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(RyderRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".listing-address a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.ryderrealestate.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(@class,'pro-chevron-right')]/parent::a/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2664)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Ryder Real Estate'
            ...
            item['Field6'] = response.xpath("//img[contains(@alt,'Bedroom')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = response.xpath("//img[contains(@alt,'Bathroom')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = response.xpath("//img[contains(@alt,'Car Park')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.listing-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-advert-container h1 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-advert-container p ::text").getall())  # description
            ...
            if response.css(".agent-wrapper"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.card-text ::text').get('').strip()
            ...
            if len(response.css(".agent-wrapper").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field24'] = agent2.css('.card-text ::text').get('').strip()
            ...
            if len(response.css(".agent-wrapper").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field29'] = agent3.css('.card-text ::text').get('').strip()
            ...
            item['Field35'] = response.css('.listing-bbc ::text').get('').strip()

            land_area = response.xpath("//img[contains(@alt,'Land Size')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath("//img[contains(@alt,'Building Size')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            if floor_area:
