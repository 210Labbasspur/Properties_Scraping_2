...
class Galldon_Sale(scrapy.Spider):
    name = "Galldon_Sale"

    url = "https://www.galldon.com.au/pages/real-estate/results?listings_postcodes=&listing_sale_method=sale&listing_suburb_search_input=&listing_category=residential&listing_price_from=&listing_price_to=&land_size_sqm_from=&land_size_sqm_to=&listing_bedrooms=default&listing_bathrooms=default"
    prefix = "https://www.galldon.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Galldon_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.galldon.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2791)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Galldon Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bed')]/text()").get('').strip().replace('Bed','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field7'] = response.xpath("//*[contains(text(),'Bath')]/text()").get('').strip().replace('Bath','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field8'] = response.xpath("//*[contains(text(),'Car')]/text()").get('').strip().replace('Car','').replace('s','').replace('\n','').replace('\xa0','')
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.inner-wrapper ::text').getall())  # description
            ...
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}".replace('Read Less','').replace('Read More','')  # description
            ...
            land_area = response.xpath("//*[contains(text(),'Total Size')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
