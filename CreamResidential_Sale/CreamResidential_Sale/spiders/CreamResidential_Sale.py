...
class CreamResidential_Sale(scrapy.Spider):
    name = "CreamResidential_Sale"

    url = "https://creamresidential.com.au/buy"
    prefix = "https://creamresidential.com.au"
    headers = {
    ...
    count = 1
    db = 'CreamResidential'
    collection = 'CreamResidential_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(CreamResidential_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://creamresidential.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2413)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cream Residential'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fal fa-fw fa-bed-alt')]/parent::*[1]/text()").getall() if e.strip())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fal fa-fw fa-bath')]/parent::*[1]/text()").getall() if e.strip())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fal fa-fw fa-car-side')]/parent::*[1]/text()").getall() if e.strip())

            price = response.css('.pp_info_header_label_blck ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace('Offers','').replace('Above','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','').replace('OffersAbove','')
            ...
            image_js = response.xpath("//*[contains(text(),'__remixContext')]/text()").get('').strip()
            # prop_images = response.xpath("//*[contains(@class,'block')]/@src").getall()
            prop_images = re.findall(r'"url":"(https://resize\.lockedoncloud\.com[^"]+)"', image_js)
            images = []
            ...
            if response.css(".pp_info_agent_blck"):
                ...
                if agent1_name:
                    ...
                    # item['Field18'] = agent1.css('div>div:nth-child(2) ::text').get('').strip()
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_img = agent1.css(".pp_info_agent_img ::attr(style)").get('').strip()
                    agent_image = re.search(r'url\((.*?)\)', agent_img).group(1) if agent_img else ''
            ...
            if len(response.css(".pp_agent_blck").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_img = agent2.css(".pp_info_agent_img ::attr(style)").get('').strip().replace('background-image:url(','').replace(')','')
                    agent_image = re.search(r'url\((.*?)\)', agent_img).group(1) if agent_img else ''
                    images_string = f'https://{agent_image}' if agent_image else ""
            ...
            if len(response.css(".pp_agent_blck").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_img = agent3.css(".pp_info_agent_img ::attr(style)").get('').strip().replace('background-image:url(','').replace(')','')
                    agent_image = re.search(r'url\((.*?)\)', agent_img).group(1) if agent_img else ''
                    images_string = f'https://{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[text()='Land']/following-sibling::*[1]/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath('//*[contains(text(),"Internal")]/following-sibling::span[1]/text()').get('').strip()
            if floor_area:
