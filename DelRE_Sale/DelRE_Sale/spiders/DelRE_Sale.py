...
class DelRE_Sale(scrapy.Spider):
    name = "DelRE_Sale"

    url = "https://www.delrealestate.com.au/search/?show_in_sale=true&sold=0&pageno={}"
    prefix = "https://www.delrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(DelRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".info a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2672)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Del ReaL Estate'

            property_street = response.xpath("//*[contains(@itemprop,'streetAddress')]/text()").get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@itemprop,'addressLocality')]/text()").getall() if e.strip())  # property_suburb
            addressRegion = response.xpath("//*[contains(@itemprop,'addressRegion')]/text()").get('').strip()  # property_street
            property_address = f"{property_street} {property_suburb} {addressRegion}"
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon-bedrooms')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bathrooms')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'icon-garage')]/parent::*[1]/text()").get('').strip()

            price = response.css('.property-title small ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','').replace('-','')
            ...
            '''          AGENTS          '''
            if response.css(".agent-detail"):
                agent1 = response.css(".agent-detail")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-detail").getall()) > 1:
                agent2 = response.css(".agent-detail")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agent-detail").getall()) > 2:
                agent3 = response.css(".agent-detail")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-phone')]/parent::*/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field35'] = response.xpath("//*[contains(@class,'icon-apartment')]/parent::*[1]/text()").get('').strip()

            land_area = response.xpath("//*[contains(@class,'icon-area')]/parent::*[1]/text()").get('').strip()
            if land_area:
