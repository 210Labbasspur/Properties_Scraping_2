...
class KempRE_Sale(scrapy.Spider):
    name = "KempRE_Sale"

    url = "https://kemprealestate.com.au/properties-for-sale"
    prefix = "https://kemprealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(KempRE_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2430)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Kemp Real Estate'

            bbc = response.css('#contentContainer')
            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in bbc.xpath(".//*[contains(@itemprop,'address')]/span/text()").getall() if e.strip())  # property_suburb
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.med-7 h2 ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".med-7 .contentRegion p ::text").getall())  # description
            ...
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('576x324','1024x576')
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(src)").get('').strip().replace('240x240','800x800')
                    images_string = f'https://kemprealestate.com.au/{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(src)").get('').strip().replace('240x240','800x800')
                    images_string = f'https://kemprealestate.com.au/{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(src)").get('').strip().replace('240x240','800x800')
                    images_string = f'https://kemprealestate.com.au/{agent_image}' if agent_image else ""
            ...
            bbc = response.css(".icons ::text").getall()
            ...
            if land_area:
                if 'Acres' in land_area:
                    ...
                    item['Field36'] = land_area.replace('sqm','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            ...
            #
            feature_count = 58
            for feature in response.xpath("//*[text()='Property Features']/following-sibling::ul//span"):   ##  first 4 features are ID, bed, bath etc.
