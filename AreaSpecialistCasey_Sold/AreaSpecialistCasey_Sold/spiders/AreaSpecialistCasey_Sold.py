...
class AreaSpecialistCasey_Sold(scrapy.Spider):
    name = "AreaSpecialistCasey_Sold"

    url = "https://www.areaspecialistcasey.com.au/pages/real-estate/results?widgetKey=122479&context=listings&returnAs=markup&listing_sale_method=sale&display_sale_method=SOLD&status=sold&pg=1"
    prefix = "https://www.areaspecialistcasey.com.au"
    headers = {
    ...
    count = 1
    db = 'AreaSpecialistCasey'
    collection = 'AreaSpecialistCasey_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(AreaSpecialistCasey_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("a.listing-item-image ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.areaspecialistcasey.com.au{each_url}'
        ...
        next_page = response.css(".more-button ::attr(hx-get)").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2673)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Area Specialist Casey'
            ...
            item['Field6'] = response.xpath("//*[text()='Bed']/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[text()='Bath']/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[text()='Car']/following-sibling::*[1]/text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.about .body ::text').getall())  # description
