...
class Merchan_Sale(scrapy.Spider):
    name = "Merchan_Sale"
    url = ('https://merchan.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bpost_type%5D%5Btype%5D=equal&query%5Bcount%5D%5Bvalue%5D=20&query%5Bcount%5D%5Btype%5D=equal&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Borderby%5D%5Btype%5D=equal&query%5Bmeta_key%5D%5Bvalue%5D=dateListed&query%5Bmeta_key%5D%5Btype%5D=equal&query%5Bsold%5D%5Bvalue%5D=0&query%5Bsold%5D%5Btype%5D=equal&query%5BsaleOrRental%5D%5Bvalue%5D=Sale&query%5BsaleOrRental%5D%5Btype%5D=equal&query%5Border%5D%5Bvalue%5D=dateListed-desc&query%5Border%5D%5Btype%5D=equal&query%5Bextended%5D%5Bvalue%5D=1&query'
           '%5Bpaged%5D%5Bvalue%5D={}&query%5Bpaged%5D%5Btype%5D=equal')
    prefix = "https://merchan.com.au"
    headers = {
    ...
    count = 1
    db = 'Merchan'
    collection = 'Merchan_Sale'
    bucket_prefix = f'Images6'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Merchan_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            item['Field12'] = ' '.join(e.strip() for e in response.css('.post-content ::text').getall()).replace('\t','').replace('\n','').replace('\r','')
