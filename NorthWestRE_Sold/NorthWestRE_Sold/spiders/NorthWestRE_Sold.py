...
class NorthWestRE_Sold(scrapy.Spider):
    name = "NorthWestRE_Sold"

    url = "https://www.northwestrealestate.net.au/recently-sold"
    prefix = "https://www.northwestrealestate.net.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(NorthWestRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".archive-top>.images-group ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.northwestrealestate.net.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(@class,'icon icon-arr-r')]/parent::a[1]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2682)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'North West Real Estate'

            property_street = response.css('h4.title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'icon-bed')]/parent::*[1]/text()").get('').strip().lower().replace('bedroom','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'icon-bath')]/parent::*[1]/text()").get('').strip().lower().replace('bathroom','').replace('s','')
            item['Field8'] = response.xpath("//*[contains(@class,'icon-garage')]/parent::*[1]/text()").get('').strip().lower().replace('car space','').replace('parking','').replace('s','')

            price = response.css('.box-price h4 ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
            item['Field11'] = response.css('a.style-2 ::text').get('').strip().replace('Sold Date:','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.single-property-desc ::text').getall())  # description
            ...
            '''          AGENTS          '''
            if response.css(".box-avatar"):
                agent1 = response.css(".box-avatar")[0]
            ...
            if len(response.css(".box-avatar").getall()) > 1:
                agent2 = response.css(".box-avatar")[1]
            ...
            if len(response.css(".box-avatar").getall()) > 2:
                agent3 = response.css(".box-avatar")[2]
            ...
            item['Field35'] = response.css('.style-1 ::text').get('').strip()

            item['Field40'] = response.css('.btn-video ::attr(href)').get('').strip()
            item['Field41'] = response.xpath("//*[@alt='img-floor']/@src").get('').strip()
            ...
            #
            feature_count = 58
            for feature in response.css(".box-feature .col-md-4"):   ##  first 4 features are ID, bed, bath etc.
