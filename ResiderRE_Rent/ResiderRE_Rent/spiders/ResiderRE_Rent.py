...
class ResiderRE_Rent(scrapy.Spider):
    name = "ResiderRE_Rent"

    url = "https://www.residerre.com.au/rent/properties-for-lease/"
    prefix = "https://www.residerre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ResiderRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".embed-responsive-item ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2581)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Resider Real Estate'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'la-bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'la-bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'la-car')]/following-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//label[contains(text(),'Price')]/following-sibling::*[1]/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@class,'slider-image')]/@href").getall()
            images = []
            ...
            item['Field35'] = response.xpath("//label[contains(text(),'Property Type')]/following-sibling::*[1]/text()").get('').strip()

            land_area = response.xpath("//label[contains(text(),'Land Size')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".description-container li.col-md-6"):   ##  first 4 features are ID, bed, bath etc.
