...
class Bombay_Rent(scrapy.Spider):
    name = "Bombay_Rent"

    url = "https://bombayre.com.au/?action=epl_search&post_type=rental&property_status=current&instance_id=1&form_tab=1"
    prefix = "https://bombayre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Bombay_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        yield scrapy.Request('https://bombayre.com.au/properties-for-rent/', headers=self.headers, callback=self.parse)
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2611)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Bombay Real Estate'

            property_street = response.css('.entry-title .item-street ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.entry-title .entry-title-sub ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@class,'epl-text-icon-value bed')]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'epl-text-icon-value bath')]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'epl-text-icon-value parking')]/text()").get('').strip()
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".single_prop_content p ::text").getall())  # description
