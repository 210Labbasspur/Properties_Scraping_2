...
class VoglWalpole_Sold(scrapy.Spider):
    name = "VoglWalpole_Sold"

    url = "https://www.voglwalpole.com.au/sold/"
    prefix = "https://www.voglwalpole.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(VoglWalpole_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".ab-link ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2688)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Vogl Walpole'
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'fa-bed')]/preceding-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@class,'fa-bath')]/preceding-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/preceding-sibling::*[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('.sale-price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.col-md-8 div.text ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'media-wrapper')]//img/@src").getall()
            images = []
            ...
            '''          AGENTS          -      FIXED        '''
            agent1_name = 'Karen Vogl'
            if agent1_name:
                first_name1, last_name1 = agent1_name.split(maxsplit=1)
                item['Field15'] = first_name1.strip()  # agent_first_name_1
                item['Field16'] = last_name1.strip()  # agent_surname_name_1
                item['Field17'] = agent1_name.strip()  # agent_full_name_1
                item['Field18'] = 'Licenced Estate Agent/Auctioneer REIV'
                item['Field19'] = 'karen@voglwalpole.com.au'
                item['Field20'] = '0438 072 370'
                item['Field22'] = 'https://s3.ap-southeast-1.wasabisys.com/Images2_1/5168048051_1.jpg'

            agent2_name = 'Donna Walpole'
            if agent2_name:
                item['Field23'] = agent2_name.strip()
                item['Field24'] = 'Licensed Estate Agent'
                item['Field25'] = 'donna@voglwalpole.com.au'
                item['Field26'] = '0421 997 827'
                item['Field27'] = 'https://s3.ap-southeast-1.wasabisys.com/Images2_1/1495466389_1.jpg'
            ...
            item['Field81'] = response.css('.inspection-link span ::text').get('').strip()
