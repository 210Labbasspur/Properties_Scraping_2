...
class KeatingEstates_Sold(scrapy.Spider):
    name = "KeatingEstates_Sold"

    url = "https://www.keatingestates.com/sold"
    prefix = "https://www.keatingestates.com"
    headers = {
    ...
    count = 1
    db = 'KeatingEstates'
    collection = 'KeatingEstates_Sold'
    ...
    def __init__(self, *args, **kwargs):
        super(KeatingEstates_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2842)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Keating Estates'
            ...
            prop_images = response.xpath("//*[contains(@class,'w-full')]/@src").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = img.replace('background-image:url(','').replace(");","")
                images.append(f'https://www.keatingestates.com{img}' if img else "")
