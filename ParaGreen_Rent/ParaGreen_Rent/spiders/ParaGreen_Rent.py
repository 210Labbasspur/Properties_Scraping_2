...
class ParaGreen_Rent(scrapy.Spider):
    name = "ParaGreen_Rent"

    url = "https://www.paragreen.com.au/for-rent/"
    prefix = "https://www.paragreen.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ParaGreen_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".wpsight-listing-title a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2548)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'ParaGreen Real Estate'
            ...
            '''          AGENTS          '''
            if response.css(".wpsight-listing-section-agent"):
                agent1 = response.css(".wpsight-listing-section-agent")[0]
            ...
            if len(response.css(".wpsight-listing-section-agent").getall()) > 1:
                agent2 = response.css(".wpsight-listing-section-agent")[1]
            ...
            if len(response.css(".wpsight-listing-section-agent").getall()) > 2:
                agent3 = response.css(".wpsight-listing-section-agent")[2]
