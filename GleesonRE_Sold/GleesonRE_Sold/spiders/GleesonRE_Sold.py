...
class GleesonRE_Sold(scrapy.Spider):
    name = "GleesonRE_Sold"

    url = "https://www.gleesonre.com.au/recently-sold/"
    prefix = "https://www.gleesonre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(GleesonRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".art-post-image a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2873)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Gleeson'

            property_street = response.css('.bipropsingle__address ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedrooms: ')]/text()").get('').strip().replace('Bedrooms: ','').replace('x','')
            item['Field7'] = response.xpath("//*[contains(text(),'Bathrooms: ')]/text()").get('').strip().replace('Bathrooms: ','').replace('x','')
            # item['Field8'] = response.xpath("//*[contains(@class,'icon-car')]/parent::*[1]/text()").get('').strip()

            price = response.css('.art-headline__description p ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.bipropsingle__content ::text').getall())  # description
            ...
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}".replae('READ MORE','')  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[contains(@class,'art-image-slider-item')]//img/@src").getall()
            images = []
            ...
            item['Field35'] = response.css('.bi-small-accent ::text').get('').strip()

            land_area = response.xpath("//*[contains(text(),'Build surface: ')]/text()").get('').strip().replace('Build surface: ','').replace('sq. m','')
            if land_area:
