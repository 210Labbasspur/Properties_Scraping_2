...
class YERE_Rent(scrapy.Spider):
    name = 'YERE_Rent'
    url = "https://www.yere.com.au/renting/properties-for-lease/"
    prefix = 'https://www.yere.com.au'
    headers = {
    ...
    count = 1
    db = 'YERE'
    collection = 'YERE_Rent'
    # bucket_prefix = f'D_{collection}'
    bucket_prefix = f'Images10'
    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(YERE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2563)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field3A'] = datetime.now().date()  # date_data_scanned as a date object
            item['Field4'] = 'Your Expert Real Estate'
            ...
            prop_images = response.xpath("//*[contains(@class,'popup-gallery slider-image')]//img/@src").getall()
            images = []
            ...
            if response.xpath("//*[contains(text(),'Building Size')]/following-sibling::div[1]/text()").get('').strip():
                land_area = response.xpath("//*[contains(text(),'Building Size')]/following-sibling::div[1]/text()").get('').strip()
                item['Field37'] = land_area.replace('sqm','').replace(' ','')   # land_area
