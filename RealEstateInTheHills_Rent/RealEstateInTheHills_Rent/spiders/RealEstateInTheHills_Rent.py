...
class RealEstateInTheHills_Rent(scrapy.Spider):
    name = "RealEstateInTheHills_Rent"

    url = "https://realestateinthehills.com/for-rent/residential-rentals/"
    prefix = "https://realestateinthehills.com"
    headers = {
    ...
    count = 1
    db = 'RealEstateInTheHills'
    collection = 'RealEstateInTheHills_Rent'
    ...
    def __init__(self, *args, **kwargs):
        super(RealEstateInTheHills_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".more-details ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2467)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Real Estate In The Hills - Sexton Glover Watts'

            property_street = response.css('h1.entry-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bedroom')]/parent::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bathroom')]/parent::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Parking')]/parent::*[1]/text()").get('').strip()

            price = response.css('.property-info .h2 ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.tile ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'enlarge')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agents .row"):
                agent1 = response.css(".agents .row")[0]
                ...
                if agent1_name:
                    ...
                    phone_nos = agent1.xpath(".//*[contains(text(),'Office')]/text()").getall()
                    for phone in phone_nos:
                        if 'Office' in phone.strip():
                            item['Field21'] = phone.strip().replace('Office:','')
                        if 'Mobile' in phone.strip():
                            item['Field20'] = phone.strip().replace('Mobile:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agents .row").getall()) > 1:
                agent2 = response.css(".agents .row")[1]
                ...
                if agent2:
                    ...
                    phone_nos = agent2.xpath(".//*[contains(text(),'Office')]/text()").getall()
                    for phone in phone_nos:
                        if 'Office' in phone.strip():
                            item['Field26'] = phone.strip().replace('Office:','')
                        if 'Mobile' in phone.strip():
                            item['Field26A'] = phone.strip().replace('Mobile:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css(".agents .row").getall()) > 2:
                agent3 = response.css(".agents .row")[2]
                ...
                if agent3:
                    ...
                    phone_nos = agent3.xpath(".//*[contains(text(),'Office')]/text()").getall()
                    for phone in phone_nos:
                        if 'Office' in phone.strip():
                            item['Field31'] = phone.strip().replace('Office:','')
                        if 'Mobile' in phone.strip():
                            item['Field31A'] = phone.strip().replace('Mobile:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            item['Field33'] = response.xpath("//*[contains(text(),'ID')]/parent::*[1]/text()").get('').strip()
            # item['Field35'] = response.css('.ididididididid ::text').get('').strip()

            land_area = response.xpath("//*[contains(text(),'Area')]/following-sibling::*[1]/text()").get('').strip()
            if land_area:
