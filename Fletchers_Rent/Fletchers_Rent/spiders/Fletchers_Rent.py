...
class Fletchers_Rent(scrapy.Spider):
    name = "Fletchers_Rent"

    url = "https://fletchers.net.au/properties-for-lease?async=1&nomap=1&ac=&page=1&office=&orderby=&lifestyle=&min=0&max=&beds=0&baths=0&cars=0&categories=&types=&radius=1&extras=&searchtype=lease"
    prefix = "https://fletchers.net.au"
    headers = {
            'Accept': 'text/javascript, text/html, application/xml, text/xml, */*',
            'Accept-Language': 'en-US,en;q=0.9',
            'Connection': 'keep-alive',
            'Referer': 'https://fletchers.net.au/recent-sales?page=2&radius=1&searchtype=sale',
    ...
    def __init__(self, *args, **kwargs):
        super(Fletchers_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("a.block ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2379)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Fletchers'
            ...
            item['Field6'] = response.xpath("//i[@class='fal fa-bed']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = response.xpath("//i[@class='fal fa-bath']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = response.xpath("//i[@class='fal fa-car']/preceding-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.muted ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h2.headline ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".contentRegion p ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//link[contains(@itemprop,'url')]/@href").getall()
            images = []
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('.jobTitle ::text').get('').strip()
                    ...
                    images_string = f'https://fletchers.net.au{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agentTile").getall()) > 1:
                ...
                agent2_name = agent2.css('.agent-name span ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.jobTitle ::text').get('').strip()
            ...
            if len(response.css(".agentTile").getall()) > 2:
                ...
                agent3_name = agent3.css('.agent-name span ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.jobTitle ::text').get('').strip()
            ...
            item['Field35'] = response.xpath("//*[@class='icons']/following-sibling::p[1]/text()").get('').strip()
            ...
            for feature in response.xpath("//*[contains(text(),'Property Features')]/following-sibling::ul//li"):   ##  first 4 features are ID, bed, bath etc.
                feature_text = ' '.join(e.strip() for e in feature.css('::text').getall())
