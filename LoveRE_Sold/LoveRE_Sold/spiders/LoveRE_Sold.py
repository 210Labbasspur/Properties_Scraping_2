...
class LoveRE_Sold(scrapy.Spider):
    name = "LoveRE_Sold"

    url = "https://www.lovere.com.au/sale/sold/"
    prefix = "https://www.lovere.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LoveRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".centerimage ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2487)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Love & Co Real Estate'

            property_street = response.css('.address ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.suburb-state ::text').getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(@class,'bed')]/span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@class,'bath')]/span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@class,'car')]/span[1]/text()").get('').strip()

            price = response.css('.price ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('-','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.charlie ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'fit-width')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".right .agent"):
                agent1 = response.css(".right .agent")[0]
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'font-size-20')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent1.xpath(".//*[contains(@class,'font-size-20')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field21'] = phone_nos[1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css("img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".right .agent").getall()) > 1:
                agent2 = response.css(".right .agent")[1]
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'font-size-20')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent2.xpath(".//*[contains(@class,'font-size-20')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field26A'] = phone_nos[1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css("img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".right .agent").getall()) > 2:
                agent3 = response.css(".right .agent")[2]
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'font-size-20')]/text()").get('').strip().replace('tel:','')
                    phone_nos = agent3.xpath(".//*[contains(@class,'font-size-20')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field31A'] = phone_nos[1].strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css("img ::attr(data-src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(@class,'land')]/span[1]/text()").get('').strip()
            if land_area:
            ...
            #
            feature_count = 58
            for feature in response.css(".features-list .active"):   ##  first 4 features are ID, bed, bath etc.
