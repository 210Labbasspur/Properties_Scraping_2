...
class ImpactRG_Sold(scrapy.Spider):
    name = "ImpactRG_Sold"

    url = "https://www.impactrealtygroup.com.au/sold-properties/"
    ...
    def __init__(self, *args, **kwargs):
        super(ImpactRG_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('h1.entry-title ::text').getall())  # description
            ...
            if response.css(".epl-author-box-container"):
                ...
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".photo ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
            ...
            if len(response.css(".epl-author-box-container").getall()) > 1:
                ...
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".photo ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".epl-author-box-container").getall()) > 2:
                ...
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".photo ::attr(src)").get('').strip().replace('background-image:url(','').replace(')','')
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[@class='fal fa-vector-square']/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m','').replace('\t','').replace('\n','').replace('\r','').replace('Sqm','').replace(' ','')   # land_area
