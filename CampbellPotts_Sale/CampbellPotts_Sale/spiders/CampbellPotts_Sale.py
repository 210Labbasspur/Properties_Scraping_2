...
class CampbellPotts_Sale(scrapy.Spider):
    name = "CampbellPotts_Sale"

    url = "http://campbellpotts.com.au/properties-for-sale"
    prefix = "http://campbellpotts.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(CampbellPotts_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2558)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Campbell Potts'

            property_street = response.css('.address-streetaddress ::text').get('').strip()  # property_street
            ...
            if response.css(".agentTile"):
                ...
                if agent1_name:
                    ...
                    images_string = f'http://campbellpotts.com.au{agent_image}' if agent_image else ""
            ...
            bbc = response.css(".highlight .icons ::text").getall()
