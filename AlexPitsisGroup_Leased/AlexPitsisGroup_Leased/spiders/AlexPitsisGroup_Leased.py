...
class AlexPitsisGroup_Leased(scrapy.Spider):
    name = "AlexPitsisGroup_Leased"

    url = "https://alexpitsisgroup.com.au/leased/?paged={}"
    prefix = "https://alexpitsisgroup.com.au"
    headers = {
    ...
    count = 1
    db = 'AlexPitsisGroup'
    collection = 'AlexPitsisGroup_Leased'
    ...
    def __init__(self, *args, **kwargs):
        super(AlexPitsisGroup_Leased, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        next_page = response.xpath("//span[text()='LOAD MORE']/text()").get('').strip()
        print("Next Page is :", next_page)
        if next_page:
            page_no += 1
            yield response.follow(self.url.format(page_no), headers=self.headers, callback=self.parse, meta={'page_no':page_no})
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(1444)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Alex Pitsis Group'
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'Bed')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field7'] = response.xpath("//*[contains(@title,'Bath')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')
            item['Field8'] = response.xpath("//*[contains(@title,'Car Park')]/*[2]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','').replace('\n','').replace('\r','').replace('\t','').replace('\xa0','')

            price = response.css('.listing-price-value ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@itemprop,'thumbnail')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".agent-card-v2"):
                agent1 = response.css(".agent-card-v2")[0]
                agent1_name = ' '.join(e.strip() for e in agent1.css('h3 ::text').getall() if e.strip())
                if agent1_name:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".agent-card-image ::attr(style)").get('').strip().replace("background-image: url('","").replace("');","")
            ...
            if len(response.css(".agent-card-v2").getall()) > 1:
                agent2 = response.css(".agent-card-v2")[1]
                agent2_name = ' '.join(e.strip() for e in agent2.css('h3 ::text').getall() if e.strip())
                if agent2:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".agent-card-image ::attr(style)").get('').strip().replace("background-image: url('","").replace("');","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            if len(response.css(".agent-card-v2").getall()) > 2:
                agent3 = response.css(".agent-card-v2")[2]
                agent3_name = ' '.join(e.strip() for e in agent3.css('h3 ::text').getall() if e.strip())
                if agent3:
                    ...
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".agent-card-image ::attr(style)").get('').strip().replace("background-image: url('","").replace("');","")
                    images_string = f'{agent_image}' if agent_image else ""
            ...
            land_area = response.xpath("//*[contains(@title,'Land Size')]/*[2]/text()").get('').strip()
            if land_area:
