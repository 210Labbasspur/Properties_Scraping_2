...
class JKEA_Sold(scrapy.Spider):
    name = "JKEA_Sold"

    url = 'https://www.jkea.com.au/search/?show_in_sale=true&sold=1&order_by=id&order=asc&pageno=1'
    prefix = "https://www.jkea.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(JKEA_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2798)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'JK Estate Agents'
            ...
            prop_images = response.xpath("//*[contains(@class,'property-gallery')]/@href").getall()
            images = []
            ...
            land_area = response.xpath("//*[contains(@class,'attr-land')]/span/text()").get('').strip().replace('m','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m²','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(@class,'attr-building')]/span/text()").get('').strip().replace('m','')
            if floor_area:
