...
class MaddisonEA_Sold(scrapy.Spider):
    name = "MaddisonEA_Sold"

    url = "https://www.maddisonestateagents.com.au/recently-sold"
    prefix = "https://www.maddisonestateagents.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MaddisonEA_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".growCaption a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.maddisonestateagents.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2631)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Maddison Estate Agents'

            property_street = response.css('.col-lg-9 h1 ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//img[contains(@src,'bedroom-black.png')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field7'] = response.xpath("//img[contains(@src,'bathroom-black.png')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags
            item['Field8'] = response.xpath("//*[contains(@class,'fa-car')]/following-sibling::text()[1]").get('').strip()     ## for bulship tags

            price = response.css('.light_writing ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']"):
                agent1 = response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']")[0]
                ...
                if agent1_name:
                    ...
                    phone_nos = agent1.xpath(".//*[contains(@class,'person-number')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field20'] = phone_nos[1].strip().replace('tel:','')
            ...
            if len(response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']").getall()) > 1:
                agent2 = response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']")[1]
                ...
                if agent2:
                    ...
                    phone_nos = agent2.xpath(".//*[contains(@class,'person-number')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field26'] = phone_nos[1].strip().replace('tel:','')
            ...
            if len(response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']").getall()) > 2:
                agent3 = response.xpath("//*[@class='image-box']/parent::div[@class='col-md-12']")[2]
                ...
                if agent3:
                    ...
                    phone_nos = agent3.xpath(".//*[contains(@class,'person-number')]/text()").getall()
                    if len(phone_nos) > 1:
                        item['Field31'] = phone_nos[1].strip().replace('tel:','')
            ...
            #
            feature_count = 58
            for feature in response.css(".single-feature"):   ##  first 4 features are ID, bed, bath etc.
