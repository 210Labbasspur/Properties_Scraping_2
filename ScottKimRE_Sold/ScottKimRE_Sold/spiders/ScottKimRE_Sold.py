...
class ScottKimRE_Sold(scrapy.Spider):
    name = "ScottKimRE_Sold"

    url = "https://scottkim.com.au/sell/sold-properties/"
    prefix = "https://scottkim.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(ScottKimRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-link a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2508)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'ScottKim Real Estate'

            property_street = response.css('#display-location h2.hc-title ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@data-tooltip,'Bedroom')]/span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field7'] = response.xpath("//*[contains(@data-tooltip,'Bathroom')]/span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')
            item['Field8'] = response.xpath("//*[contains(@data-tooltip,'car space')]/span[1]/text()").get('').strip().lower().replace('bed','').replace('s','').replace('bath','').replace('car','').replace('parking','')

            price = response.css('h3.hc-title>span ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css("#listing-description-content ::text").getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'slide-image')]/@style").getall()
            images = []
            ...
            item['Field35'] = response.css('.type ::text').get('').strip()

            land_area = response.css(".land_size span ::text").get('').strip()
            if land_area:
