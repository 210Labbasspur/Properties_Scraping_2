...
class MilesRE_Sold(scrapy.Spider):
    name = "MilesRE_Sold"

    url = "https://www.milesre.com.au/recently-sold"
    prefix = "https://www.milesre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(MilesRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".property-card a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://www.milesre.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2783)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Miles Real Estate'

            property_street = response.css('h1.inside-heading ::text').get('').strip()  # property_street
            ...
            bbc = response.css('.property-details-amenities-wrapper')
            item['Field6'] = bbc.xpath("//*[contains(@class,'icon-bed')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = bbc.xpath("//*[contains(@class,'icon-bath')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = bbc.xpath("//*[contains(@class,'icon-car')]/following-sibling::*[1]/text()").get('').strip()

            price = response.xpath("//*[contains(text(),'PRICE')]/parent::*/following-sibling::*[1]/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-details .bottom-pad:nth-child(1) p ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'slide-expand')]/@href").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='property-details-agent-details']/parent::*"):
                agent1 = response.xpath("//*[@class='property-details-agent-details']/parent::*")[0]
                ...
                if agent1_name:
                    ...
                    item['Field18'] = agent1.css('strong ::text').get('').strip()
            ...
            if len(response.xpath("//*[@class='property-details-agent-details']/parent::*").getall()) > 1:
                agent2 = response.xpath("//*[@class='property-details-agent-details']/parent::*")[1]
                agent2_name = agent2.css('.agent-name ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('strong ::text').get('').strip()
            ...
            if len(response.xpath("//*[@class='property-details-agent-details']/parent::*").getall()) > 2:
                agent3 = response.xpath("//*[@class='property-details-agent-details']/parent::*")[2]
                agent3_name = agent3.css('.agent-name ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('strong ::text').get('').strip()
            ...
            land_area = response.xpath('//*[contains(text(),"LAND")]/parent::*/following-sibling::*[1]/text()').get('').strip().replace('sq. metres approx.','').replace('is','').replace('m²','')
            if land_area:
