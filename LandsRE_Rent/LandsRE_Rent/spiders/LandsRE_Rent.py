...
class LandsRE_Rent(scrapy.Spider):
    name = "LandsRE_Rent"

    url = "https://landsrealestate.com.au/rent?p.1.offset=1"
    prefix = "https://landsrealestate.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LandsRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://landsrealestate.com.au{each_url}'
        ...
        next_page = response.xpath("//*[text()='>']/parent::*/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2434)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Lands Real Estate'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-bed-alt')]/parent::*[1]/text()").getall())
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-fw fa-bath')]/parent::*[1]/text()").getall())
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,'fa-car-side')]/parent::*[1]/text()").getall())

            price = response.xpath("//*[contains(@class,'pp_info_header_label_blck')]/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            ...
            prop_images = response.xpath("//*[contains(@style,');cursor:pointer')]/@style").getall()
            images = []
            for img in prop_images[:25]:        ## Scrape maximum 25 photos
                img = re.search(r'url\((.*?)\)', img).group(1)
                # img = img.replace('background-image:url(','').replace(");","")
                images.append(f'https://landsrealestate.com.au{img}' if img else "")
            ...
            '''          AGENTS          '''
            if response.css(".pp_agent_blck"):
                agent1 = response.css(".pp_agent_blck")[0]
                agent1_name = agent1.xpath(".//div[contains(@style,'color:#21252A')]/text()").get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.xpath(f".//*[contains(text(),'{agent1_name}')]/following-sibling::*[1]/text()").get('').strip()
                    item['Field19'] = agent1.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent1.css(".pp_agent_img ::attr(style)").get('').strip()
                    img = re.search(r'url\((.*?)\)', agent_image).group(1)
                    images_string = f'https://landsrealestate.com.au{img}' if agent_image else ""
            ...
            if len(response.css(".pp_agent_blck").getall()) > 1:
                ...
                agent2_name = agent2.xpath(".//div[contains(@style,'color:#21252A')]/text()").get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.xpath(f".//*[contains(text(),'{agent2_name}')]/following-sibling::*[1]/text()").get('').strip()
                    item['Field25'] = agent2.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent2.css(".pp_agent_img ::attr(style)").get('').strip()
                    img = re.search(r'url\((.*?)\)', agent_image).group(1)
                    images_string = f'https://landsrealestate.com.au{img}' if agent_image else ""
            ...
            if len(response.css(".pp_agent_blck").getall()) > 2:
                ...
                agent3_name = agent3.xpath(".//div[contains(@style,'color:#21252A')]/text()").get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.xpath(f".//*[contains(text(),'{agent3_name}')]/following-sibling::*[1]/text()").get('').strip()
                    item['Field30'] = agent3.xpath(".//*[contains(@class,'fal fa-fw fa-at')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('mailto:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
                    agent_image = agent3.css(".pp_agent_img ::attr(style)").get('').strip()
                    img = re.search(r'url\((.*?)\)', agent_image).group(1)
                    images_string = f'https://landsrealestate.com.au{img}' if agent_image else ""
            ...
            land = response.xpath("//*[text()='Land']/following-sibling::*[1]/text()").get('').strip()
