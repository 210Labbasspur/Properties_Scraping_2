...
class FollettCo_Rent(scrapy.Spider):
    name = "FollettCo_Rent"

    url = "https://follettco.com.au/lease/"
    prefix = "https://follettco.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(FollettCo_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def start_requests(self):
        ...
        for each_url in all_links:
            each_url = f'https://follettco.com.au{each_url}'
        ...
        for property_url in all_links:
            property_url = f'https://follettco.com.au{property_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2546)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Follett & Co'

            property_street = response.css('.property-header__left h1 ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.property-template__content h1 ::text').getall() if e.strip())  # property_suburb
            property_address = f"{property_street} {property_suburb}"
            item['Field5'] = property_address  # full_adddress

            item['Field6'] = response.xpath("//*[contains(@d,'M19.0664 11.2181H10.3136V10.5908H19.3727V11.2181H19')]/ancestor::span[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@d,'M18.7583 8.82431C18.0449 11.5192 ')]/ancestor::span[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@d,'M17.5037 8.14921C17.5037 ')]/ancestor::span[1]/text()").get('').strip()

            price = response.css('.property-template__description h4 ::text').get('').strip().replace('Sold','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')
                # item['Field11'] = 'sold_date'

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.property-template__description h4:nth-child(2) ::text').getall())  # description
            detailed_desc = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css(".property-template__text ::text").getall())  # description
            if description_head or detailed_desc:
                item['Field12'] = f"{description_head} {detailed_desc}"  # description

            '''         Uploading Images on Wasabi S3 Bucket            '''
            prop_images = response.xpath("//*[@property='og:image']/@content").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.css(".property-template__agent"):
                agent1 = response.css(".property-template__agent")[0]
            ...
            if len(response.css(".property-template__agent").getall()) > 1:
                agent2 = response.css(".property-template__agent")[1]
            ...
            if len(response.css(".property-template__agent").getall()) > 2:
                agent3 = response.css(".property-template__agent")[2]
