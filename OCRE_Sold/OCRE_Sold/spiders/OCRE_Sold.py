...
class OCRE_Sold(scrapy.Spider):
    name = "OCRE_Sold"

    url = "https://ocre.com.au/property/?action=epl_search&post_type=property&property_status=sold&sortby=sold_latest?timms_redirected=true"
    prefix = "https://ocre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(OCRE_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        all_links = response.xpath("//*[contains(@class,'polaroid-property')]//a/@href").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2459)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Ouwens Casserly Real Estate'

            property_street = response.css('.xxxxxxxxxxxxxx ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.xpath("//*[contains(@class,'entry-title')]//text()").getall() if e.strip())  # property_suburb
            ...
            item['Field6'] = response.xpath("//*[contains(text(),'Bed')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(text(),'Bath')]/preceding-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(text(),'Car')]/preceding-sibling::*[1]/text()").get('').strip()
            ...
            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.xpath("//*[contains(@class,'offset-md-1')]//text()").getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@data-fancybox,'gallery')]/@data-original-image").getall()
            images = []
            ...
            '''          AGENTS          '''
            # if response.css("#fancy-agents"):
            if response.xpath("//*[contains(@id,'fancy-agents')]"):
                agent1 = response.xpath("//*[contains(@id,'fancy-agents')]")[0]
                agent1_name = agent1.css('.mb-0 a ::text').get('').strip()
                if agent1_name:
                    first_name1, last_name1 = agent1_name.split(maxsplit=1)
                    item['Field15'] = first_name1.strip()  # agent_first_name_1
                    item['Field16'] = last_name1.strip()  # agent_surname_name_1
                    item['Field17'] = agent1_name.strip()  # agent_full_name_1
                    item['Field18'] = agent1.css('.mb-2 ::text').get('').strip()
            ...
            if len(response.xpath("//*[contains(@id,'fancy-agents')]").getall()) > 1:
                agent2 = response.xpath("//*[contains(@id,'fancy-agents')]")[1]
                agent2_name = agent2.css('.mb-0 a ::text').get('').strip()
                if agent2:
                    item['Field23'] = agent2_name.strip()
                    item['Field24'] = agent2.css('.mb-2 ::text').get('').strip()
            ...
            if len(response.xpath("//*[contains(@id,'fancy-agents')]").getall()) > 2:
                agent3 = response.xpath("//*[contains(@id,'fancy-agents')]")[2]
                agent3_name = agent3.css('.mb-0 a ::text').get('').strip()
                if agent3:
                    item['Field28'] = agent3_name.strip()
                    item['Field29'] = agent3.css('.mb-2 ::text').get('').strip()
            ...
            land_area = response.xpath("//*[contains(text(),'Land Size')]/span/text()").get('').strip()
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('Land','').replace('is','').replace('m','').replace('sqm','').replace('Sqm','').replace(' ','')   # land_area
            floor_area = response.xpath("//*[contains(text(),'Building Size')]/span/text()").get('').strip()
            if floor_area:
