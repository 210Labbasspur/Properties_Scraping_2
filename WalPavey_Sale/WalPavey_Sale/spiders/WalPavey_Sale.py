...
class WalPavey_Sale(scrapy.Spider):
    name = "WalPavey_Sale"

    url = "https://www.walpavey.com.au/show-all-properties-page-1"
    prefix = "https://www.walpavey.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(WalPavey_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f"https://www.walpavey.com.au{each_url.replace('..','')}"
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2340)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wal Pavey Real Estate'

            property_street = response.css('.prop-title span ::text').get('').strip()  # property_street
            ...
            item['Field8'] = response.xpath("//*[contains(text(),'Garage:')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('h2.prop-title ::text').get('').strip().replace('Sold','').replace('For','').replace('$','').replace(':','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.desc-holder ::text').getall())  # description
            ...
            item['Field15'] = 'Wal'
            item['Field16'] = 'Pavey'
            item['Field17'] = 'Wal Pavey'
            item['Field19'] = 'wal@walpavey.com.au'
            item['Field20'] = '0408 062 858'
            '''         Uploading Images on Wasabi S3 Bucket            '''
            agent_image = 'https://admin.commercialpremises.com.au/userimgs/m2490/agents/20201230104930-1.jpg'
            ...
            item['Field23'] = 'Cindy Pavey'
            item['Field25'] = 'cindy@walpavey.com.au'
            item['Field26'] = '0421649783'
            '''         Uploading Images on Wasabi S3 Bucket            '''
            agent_image = 'https://admin.commercialpremises.com.au/userimgs/m2490/agents/20201230105028-1.jpg'
            images_string = f'{agent_image}' if agent_image else ""
            ...
            land = response.xpath("//*[contains(text(),'Land size:')]/following-sibling::*[1]/text()").get('').strip()
            land_area = land.replace('Land','').replace('is','').replace('Metres²','')
            if land_area:
                if 'Acres' in land_area:
                    pass
                else:
                    item['Field36'] = land_area.replace('m²','').replace(' ','')   # land_area
            building = response.xpath("//*[contains(text(),'Building size:')]/following-sibling::*[1]/text()").get('').strip()
            building_area = building.replace('Land','').replace('is','').replace('Metres²','')
            if building_area:
                if 'Acres' in building_area:
                    pass
                else:
                    item['Field37'] = building_area.replace('m²','').replace(' ','')   # land_area
            # feature_count = 58
