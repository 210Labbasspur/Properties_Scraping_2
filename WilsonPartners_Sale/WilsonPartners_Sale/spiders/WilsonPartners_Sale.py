...
class WilsonPartners_Sale(scrapy.Spider):
    name = "WilsonPartners_Sale"

    url = "https://www.wilsonpartners.com.au/buying/search-properties/"
    prefix = "https://www.wilsonpartners.com.au"
    headers = {
    ...
    count = 1
    db = 'WilsonPartners'
    collection = 'WilsonPartners_Sale'
    ...
    def __init__(self, *args, **kwargs):
        super(WilsonPartners_Sale, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css("#thumbnail-mode .image a ::attr(href)").getall()
        links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2554)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Wilson Partners'

            property_street = response.css('.street_address .value ::text').get('').strip()  # property_street
            property_suburb = ' '.join(e.strip() for e in response.css('.suburb .value ::text').getall() if e.strip())  # property_suburb
            postcode = response.css('.postcode .value ::text').get('').strip()  # property_street
            property_address = f"{property_street} {property_suburb} {postcode}"
            ...
            price = response.xpath("//*[contains(text(),'Price')]/following-sibling::*[1]/text()").get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            description_head = ' '.join(element.strip().replace('\t', ' ').replace('\r', ' ').replace('\n', ' ') for element in response.css('.heading .title ::text').getall())  # description
            ...
            prop_images = response.xpath("//*[contains(@class,'container')]/@style").getall()
            images = []
            ...
            if response.css("#property-enquiry .agent"):
                ...
                if agent1_name:
                    ...
                    item['Field20'] = agent1.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    item['Field21'] = agent1.xpath(".//*[contains(@class,'fa-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("#property-enquiry .agent").getall()) > 1:
                ...
                if agent2:
                    ...
                    item['Field26'] = agent2.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    item['Field26A'] = agent2.xpath(".//*[contains(@class,'fa-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
            ...
            if len(response.css("#property-enquiry .agent").getall()) > 2:
                ...
                if agent3:
                    ...
                    item['Field31'] = agent3.xpath(".//*[contains(@class,'fa-mobile')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    item['Field31A'] = agent3.xpath(".//*[contains(@class,'fa-phone')]/parent::*[1]/text()").get('').strip().replace('tel:','')
                    '''         Uploading Images on Wasabi S3 Bucket            '''
