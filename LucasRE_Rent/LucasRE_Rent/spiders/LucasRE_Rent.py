...
class LucasRE_Rent(scrapy.Spider):
    name = "LucasRE_Rent"

    url = "https://www.lucasre.com.au/pages/real-estate/results?widgetKey=203169&context=listings&returnAs=markup&listing_category=Residential&listing_sale_method=lease&display_sale_method=RENT&pg=1"
    prefix = "https://www.lucasre.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(LucasRE_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        ...
        for each_url in all_links:
            each_url = f'https://www.lucasre.com.au{each_url}'
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2681)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Lucas Real Estate'
            ...
            item['Field6'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,' bed')]/text()").getall() if e.strip()).replace('Bed','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field7'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,' bath')]/text()").getall() if e.strip()).replace('Bath','').replace('s','').replace('\n','').replace('\xa0','')
            item['Field8'] = ''.join(e.strip() for e in response.xpath("//*[contains(@class,' car')]/text()").getall() if e.strip()).replace('Car','').replace('s','').replace('\n','').replace('\xa0','')

            price = response.css('.price-heading ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
