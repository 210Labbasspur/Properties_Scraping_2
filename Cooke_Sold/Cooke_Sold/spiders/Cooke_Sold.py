...
class Cooke_Sold(scrapy.Spider):
    name = "Cooke_Sold"
    url = ('https://www.cookeproperty.com.au/wp-admin/admin-ajax.php?action=get_posts&query%5Bpost_type%5D%5Bvalue%5D=listings&query%5Bcount%5D%5Bvalue%5D=20&query%5Borderby%5D%5Bvalue%5D=meta_value&query%5Bmeta_key%5D%5Bvalue%5D=saleDate&query%5Bsold%5D%5Bvalue%5D=1&query%5Bsold%5D%5Btype%5D=equal&query%5BsaleOrRental%5D%5Bvalue%5D=Sale&query%5BsaleOrRental%5D%5Btype%5D=equal&query%5Bdoing_wp_cron%5D%5Bvalue%5D=1740201216.9656438827514648437500&query%5Bdoing_wp_cron%5D%5Btype%5D=equal&query'
           '%5Bpaged%5D%5Bvalue%5D={}')
    prefix = "https://www.cookeproperty.com.au"
    headers = {
    ...
    count = 1
    db = 'Cooke'
    collection = 'Cooke_Sold'
    bucket_prefix = f'Images6'

    local_file_path = '/img'

    def __init__(self, *args, **kwargs):
        super(Cooke_Sold, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2014)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Cooke Property Agents'
