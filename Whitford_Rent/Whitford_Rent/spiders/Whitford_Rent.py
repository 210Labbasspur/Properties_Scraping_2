...
class Whitford_Rent(scrapy.Spider):
    name = "Whitford_Rent"

    url = "https://whitfordproperty.com.au/listings/rent"
    prefix = "https://whitfordproperty.com.au"
    headers = {
    ...
    def __init__(self, *args, **kwargs):
        super(Whitford_Rent, self).__init__(*args, **kwargs)
        self.links = []
    ...
    def parse(self, response):
        '''    FARHAN'S LOGIC    '''
        all_links = response.css(".overflow-hidden>a ::attr(href)").getall()
        links = []
        for each_url in all_links:
            each_url = f'https://whitfordproperty.com.au{each_url}'
        ...
        next_page = response.xpath("//*[contains(@class,'relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-secondary bg-white border border-gray-300 cursor-pointer rounded-r-md leading-5 hover:opacity-60')]/@href").get('').strip()
    ...
    def detail_parse(self, response):
        ...
        if detail_page_url not in database_sale_matching_url:
            item = dict()
            item['Field1'] = ''
            item['Field2'] = int(2862)
            item['Field3'] = datetime.now().date().strftime('%Y-%m-%d')  # date_data_scanned
            item['Field4'] = 'Whitford Property'

            property_street = response.css('h1.font-bold ::text').get('').strip()  # property_street
            property_suburb = response.css('.px-4 .justify-between p ::text').get('').strip()  # property_street
            ...
            item['Field6'] = response.xpath("//*[contains(@title,'bedroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field7'] = response.xpath("//*[contains(@title,'bathroom')]/following-sibling::*[1]/text()").get('').strip()
            item['Field8'] = response.xpath("//*[contains(@title,'parking')]/following-sibling::*[1]/text()").get('').strip()

            price = response.css('.mt-2 h2.font-bold ::text').get('').strip().replace('Sold','').replace('for','').replace('$','').replace(' ','').replace(',','')
            if re.search(r'\d', price):
                item['Field9'] = price.strip().replace('SOLD','').replace('$','').replace('Contact','').replace('Agent','')

            desc = response.xpath("//*[contains(@class,'cms-text bg-white lg:py-0 py-4')]")
            ...
            prop_images = response.xpath("//*[contains(@class,'w-full h-full object-cover object-center')]/@src").getall()
            images = []
            ...
            '''          AGENTS          '''
            if response.xpath("//*[@class='flex py-2']"):
                agent1 = response.xpath("//*[@class='flex py-2']")[0]
            ...
            if len(response.xpath("//*[@class='flex py-2']").getall()) > 1:
                agent2 = response.xpath("//*[@class='flex py-2']")[1]
            ...
            if len(response.xpath("//*[@class='flex py-2']").getall()) > 2:
                agent3 = response.xpath("//*[@class='flex py-2']")[2]
